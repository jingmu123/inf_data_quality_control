...
pattern_en = [
    ...
    [r'(^(We thank|Special thanks?|References values|\d?Department of).*)', r'删除12:<u>\1</u>'],
    [r'([^\.]*(work is properly|is properly cited).*)', r''],
    [r'(^(Clinical ?[Il]mage|Clinical ?Video)$)', r'删除14:<u>\1</u>'],
    [r'(^Video \d+.*)', r'删除15:<u>\1</u>'],
    [r'(^\| [Ff]igs?(ure)?[\w\W]*)', r'删除16:<u>\1</u>'],
    [r'(^(\(\w+\) ?\(\w+\)|\d\w( \d\w)+|[\d\-]+：)$)', r'删除17:<u>\1</u>'],
    [r'(^(.{0,250})\d{4}[;；：\.]\w+(\(\d+\))?[:：；\.]?[\w \-]*.{0,50}$)', r'删除18:<u>\1</u>'],
...
fw = open(r"C:/Program Files/lk/projects/pdf/ccri/ccri_preformat_clean-test.jsonl", "w", encoding="utf-8")
