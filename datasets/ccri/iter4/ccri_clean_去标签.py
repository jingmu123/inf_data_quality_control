...
pattern_en = [
    ...
    # ========================================================================================
    # 以下补充对此组数据清洗的特定正则
    [r'(^[ \\]*(Correspondence：|OPEN ACCESS|E-mail|Copyright).*)', r''],
    [r'(^#*\s?([Ff]igs?(ure)?|F\s?IGS?(URE)?).*)', r''],  # 删除开头为Figure的描述
    [r'(^[ \\]*(([A-Z][a-z]+( \w+){0,2})[,，] ?){2,}([A-Z][a-z]+( \w+){0,2})[ \\]*.*)', r''],
    [r'^([\d,，\-\–— \.]+)$', r''],
    [r'(^\d+\\?\..*\d{4}[;；：\.]\w+(\(\d+\))?[:：；\.]?[\w \-]+.*)', r''],
    [r'(^\d+\\?\..*\d{4}[;；：\.]$)', r''],
    [r'(.*((Received|Accepted|Published) Date：|E-mail：).*)', r''],
    [r'(^(ISSN：).*)', r''],
    [r'(^(is properly cited|cited|\d+(：\d+)*|[\w \)]{1,5})[\.：\s]*$)', r''],
    [r'(\.)([\d；：\. ]*(ISSN：).*)',  r'\1'],
    [r'(^(We thank|Special thanks?|References values|\d?Department of).*)', r''],
    [r'([^\.]*(work is properly|is properly cited).*)', r''],
    [r'(^(Clinical ?[Il]mage|Clinical ?Video)$)', r''],
    [r'(^Video \d+.*)', r''],
    [r'(^\| [Ff]igs?(ure)?[\w\W]*)', r''],
    [r'(^(\(\w+\) ?\(\w+\)|\d\w( \d\w)+|[\d\-]+：)$)', r''],
    [r'(^(.{0,250})\d{4}[;；：\.]\w+(\(\d+\))?[:：；\.]?[\w \-]*.{0,50}$)', r''],
    [r'( ?[，\-\|]*(\d+[，\-\|])+([1-9]\d*)?(\.|$))', r'\4'],
    [r'(^(Hao-Chiang|Ali S12\.|2CholistanUniversityy of|Pineta Grande Hospital|The data used in this study|Youth Declare War|David Atallahiand|Maheswari VU，|Stray Cats New|Alassane Tourel，|Heraldo CB Inforzato，).*)', r''],
    [r'(^(Fekede\.|India，|Kamble.|Elbey MAand UI Haq F2|Maheswari VUand Rajanikanth A|Dormon P|W13D L：43|Sibin Nair|Brazil\.)$)', r'']
...
pattern_more_line_feed = [
    ...
    [r'\s(of|in|and|or|not|the|a|any|some|for|is|are|The|A|with|&|And|their|his|her|from|on)\s?#*$', r'^#*(?!Table)'],   #  上一段结尾是A或者The 下一段一定要连上
...
def clean_text(context, lang):
    ...
    cp = clean_pattern()
    sp = speicalProces()
    context = re.sub(r'([\u4e00-\u9fff]+ | [\u4e00-\u9fff]+)', r'', context)
