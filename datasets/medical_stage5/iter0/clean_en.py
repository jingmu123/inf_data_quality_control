...
class speicalProces:
    ...
    def is_fig(self,block_text):
        if re.search('^[\s•]{0,3}([Ff]igs?(ure)?s?|FIGS?(URE)?s?)',block_text):
            return True
...
fw = open("C:/pycharm/orc识别pdf清洗数据/pdf/clean_json/reclean1_medical_stage5_en.jsonl", "w", encoding="utf-8")
with open("C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\medical_stage5_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    for items in tqdm(fs.readlines()):
        item = json.loads(items.strip())
        # if item["seq_id"] == "cf988269-a4cc-4df6-b46b-dfe66f95410a":
        # print(item)
        context = item
