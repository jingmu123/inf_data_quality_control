...
pattern_list_zh = [
    ...
    [r'([\u4e00-\u9fa5。？！)] *)(\d\.(\d{1,2}\.){0,10}\d?)( {1,10}[^\da-zA-Z\|%])', r'\1\n增加换行6:\2\4'],
    # [r'^(?!\n)([#•]{1,4}\s{,4})', r'\n增加换行7:_\1_'],
    # [r'([。\u4e00-\u9fa5] *)(\d{1,2}\.)', r'\1\n增加换行10:\2'],
    [r'。 ? *?([一二三四五六七八九十]+[\.、])', r'\n增加换行9:\1'],
    # -------增加换行：(7)
    ...
    [r'(\s?(👍|▶|●|©|®|\(?↑\)?|†|¶|║|§|∧|™|■|◆|♕|ኤ|♦|ż|❏|□|✓|✔|❍|▲|⭐|◎|●|۞|\s__\s|￥|∩|‡|♂|¿|☞|⌒|�|♀|「|\s…\s)\s?)', r'删除28:<u>\1</u>'],
    [r'((（|\(|〈|\[) ?(图|见图|表|见表|困|参见|至图|附图|友) ?(\d+[-|–\.\/,\s、~]+)+(\d+)?([^\n.。\u4e00-\u9fa5\d]*(）|\)|〉|\])))',
     r'删除29:<u>\1</u>'],
...
class speicalProces:
    ...
    def step4_rm_cite(self,context_raw):
        ...
        try:
            ...
            name_num, name_lens = len(name_list), len(''.join(name_list) + ''.join(ORG_list))
    ...
    def is_complete(self, last_sentence, curr_sentence):
        ...
        # elif "#" in last_sentence or "#" in curr_sentence:
            ...
            return True
        elif len(re.findall(r'[a-zA-Z\d]$(?<!g)', last_sentence)) > 0 or len(re.findall(r'^[a-zA-Z\d][^\.]', curr_sentence)) > 0:
            return False
    ...
    def step9_remove_en_noise(self,text):
        ...
        target = re.sub(r'\(.*?\)', '', text)
        # print('原字符：',text)
        # print('---------------------------------------------------')
        # print('替换后：',target)
        en_lis = re.findall(
            r'(([A-ZÀ-ÖØ-ßА-ЯŞĆĂÂĐÊÔƠƯÞÐÆØÅa-zà-öø-ÿа-яşćăâđêôơưþðæøåभयेर ]{2,}) ?[\d]* ?([*A-ZÀ-ÖØ-ßА-ЯŞĆĂÂĐÊÔƠƯÞÐÆØÅa-zà-öø-ÿа-яşćăâđêôơưþðæøåभयेर \d]+)[A-ZÀ-ÖØ-ßА-ЯŞĆĂÂĐÊÔƠƯÞÐÆØÅa-zà-öø-ÿа-яşćăâđêôơưþðæøåभयेर])',
            target)
        for i in en_lis:
        ...
        for word in suspect_lis:
            ...
            if word_copy.isupper() and ' ' not in word_copy:
                continue
...
with open(r'F:\zeroshots\1.jsonl', "r", encoding="utf-8") as file:
    for item in tqdm(file.readlines()):
        sp=speicalProces()
        item=process_line(item,sp)
        # print(3,item)
        with open(r'F:\zeroshots\medicalpdfv2_0724_preformat_zh_reclean1.jsonl', 'a', encoding='utf-8') as f:
            f.write(item+'\n')
