...
pattern_list_zh = [
                   r'文中有.*具体指导。',
                   r'\((基础篇|高级篇|Beyond the Basics|)\)',
                   r'\(参见.*\)',
                   r"(((基础|高级)篇)?(\(参见.*\))?)|(患者教育：.*(\(基础篇\))?)|(Patient education:.*)",
                   r'\[\d+([,，、-]\d+)*\]',
                   r'(\(参见\s*[^（]+(\([^)]*\))?[^)]*\))|(参见网页\(here\))',
                   r'关于.*参见.*(\n.*[：:].*)*',
...
#读jsonl
fw = open("full_uptodate_clean.jsonl", "w", encoding="utf-8")
with open("full_data/full_uptodate.jsonl", "r", encoding="utf-8") as fs:
