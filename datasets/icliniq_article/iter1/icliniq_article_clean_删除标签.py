...
pattern_list = [
    [r'(.*Related Topics[^$]*)',r''],     # 相关话题...
    [r'(^For more information.*)',r'']   # 欲了解更多
        ...
        for index,item in enumerate(context):
            ...
            if re.search(r'\n\s{4}Related Topics',item):
                ...
                for i in splited_item:
                    ...
                    new_item.append(i)
                item = '\n'.join(new_item)
            if Related_Topics and (re.search(r'^[\*\s]{2,10}$',item) or re.search(r'\?$',item.strip()) or index <= start_index+4):
                # item = "无关删除-2:<u>{}</u>".format(item)
                item = ""
            else:
                Related_Topics = False
        ...
        while index < len(context):
            ...
            if index >= 0:
                if index + 1 < len(context) and re.search(r'^#+\s?(\*+)?\d+\.(\*+)?$',stripped_item):
                    context[index] = stripped_item +" "+re.sub("#+\s",r'',context[index+1])
                    # 删除下一个 item
                    del context[index + 1]
                    index = index - 1
                    continue
...
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\icliniq_article_preformat.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        # if item["seq_id"] == "91f5e0bf-dcda-4842-bc2f-c03d25c8bb2b":
        context = item["text"]
        lang = item["lang"]
        title = item["title"]
