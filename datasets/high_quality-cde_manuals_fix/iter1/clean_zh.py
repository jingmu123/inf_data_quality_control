...
def clean_text(context, lang):
    ...
    for pattern_item in pattern_zh_list_2:
        context = re.sub(pattern_item[0], pattern_item[1], context)
...
fw = open(r"C:\Users\Administrator\PycharmProjects\untitled\four mission\high_quality-cde_manuals_fix\reclean1_high_quality-cde_manuals_fix.jsonl", "w", encoding="utf-8")
with open(r"C:\Users\Administrator\PycharmProjects\untitled\four mission\high_quality-cde_manuals_fix\high_quality-cde_manuals_fix_new.jsonl", "r", encoding="utf-8") as fs:
# with open(r"C:\Users\Administrator\PycharmProjects\untitled\four mission\high_quality-cde_manuals_fix\test.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        specification_1 = re.findall('(?:\n|^)(?:[* ]*规格[ *]*[:：])(.+)', context)
        specification_2 = re.findall('(?:\n|^)(?:[# ]*【规格】\n+)([^#]*)', context)


        if specification_1 != []:
            specification_1[0] = re.sub(r'((?:^(?:\n|#| )+)|(?:(?:\n|#| )+$))', '', specification_1[0])
            item['attr']['specification'] = specification_1[0]
        elif specification_2 != []:
            specification_2[0] = re.sub(r'((?:^(?:\n|#| )+)|(?:(?:\n|#| )+$))', '', specification_2[0])
            item['attr']['specification'] = specification_2[0]
        else:
