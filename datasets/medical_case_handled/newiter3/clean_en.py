...
pattern_list_en = [
    ...
    [r'(VARICOSE VEIN AND VENOUS ULCER \d+|SWELLINGS IN THE NECK \d+|\d+ A TEXTBOOK ON SURGICAL SHORT CASES|TESTIS, EPIDIDYMIS AND SCROTUM *\d+ *|\d* *SWOLLEN LEG *\d*|SYNDROME AND CERVICAL RIB *\d+|DISEASE, THORACIC OUTLET *\d+)',r''],
    [r'(?:\n)(\** *_*[A-Z]\.)(\\\])',r'\1'],
...
class speicalProces:
    ...
    def is_complete(self, last_sentence, curr_sentence):
        ...
        if len(last_sentence) == 0:
            return True
        if len(re.findall(r'(i\.e\.)',last_sentence[-4:]))>1:
            return False
...
fw = open(r"C:\Users\Administrator\PycharmProjects\untitled\medical_case_handled\reclean3_medical_case_handled.jsonl", "a", encoding="utf-8")
