...
pattern_en = [
    ...
    #09.23继续添加
    [r'(^[#\*_·]*\s?([Ff]i[qg]s?(ure)?|F\s?IGS?(URE)?).*\n?.*)', r'通用删除16(英):<u>\1</u>'],  # 删除开头为Figure的描述
    ...
    [r'(^[\\\*_ #]*(\d+|[A-Z]{1,2}|SUGGESTED READING|Suggested Reading|ACKNOWLEDGMENTS?|Acknowledge?ments?|I. INTRODUCTION|[Rr][Ee][Ff][Ee][Rr][Ee][Nn][Cc] ?[Ee][Ss]?|(Selected )?References?|FURTHER READING|Further reading( list)?|5\'\-\-CACGTAAGCTATGCAGGCTT\-\-3\'|Useful websites)[\*_]*$)',  r'删除5:<u>\1</u>'],  # 参考文献标题及穿插的标题
    ...
    [r'(^[\*_]*(Also[,，]? [Ss]ee [Cc]hapter|For more details?[,，]|Acknowledgements are|We (also )?thank|This work was|ISBN|\.com\/books).*)', r'删除12:<u>\1</u>'],  # 一些特殊无关段
    [r'(^[\*_]*((Data )?[Ff]rom：|Source：|Citation：).*)', r'删除13:<u>\1</u>'],  # 文献来源
    [r'([a-z]\.)( ?[\|\d]+([， \.\-\|]\d*)*$)', r'\1删除14:<u>\2</u>'],  # 段末无关数字
    [r'((^(?=.{0,150}$).*(Department|University).*)|(^(?=.{0,300}$).*(Depa[ri]tment of|Tel：|@[a-z]{2,10}\.com).*))', r'删除15:<u>\1</u>'],  # 部门介绍
    ]
...
start_end_list = [
    ...
    ['d5577941-ae84-44fd-a89a-4b64c772ac54', r'\*\*HISTORY\*\*', r'\*\*带SUGGESTED READING\*\*'],
...
class clean_pattern:
    ...
    def delete_page_middle(self, context, lang):
        ...
        """
        start_to_end_en = [
            [r'(^[\*_ ]*(ACKNOWLEDGE?MENTS?|Acknowledge?ments?)[\*_ ]*$)', r'(^[#\*_\s]*([A-Z][a-zA-Z]+( [a-zA-Z]+){0,8})：?[_\*\s]*$)', 0],
        ]
        start_to_end_zh = [
            # 样例
...
class speicalProces:
    ...
    def is_ref(self, con):
        flag = False
        patterns = [
            r'^[\*\.]*[\d _]*(_\d+_ ?|\d+\\?[\.，][^\d]|[A-Ze][A-Z\-\'a-z]+[， ,]?[A-Z ]{1,4}[\.，,：\(])',  # 参考文献开头，序号或者人名
            r'([\s\.，,]+et al?[\.:：，,]+)',  # et al
        ...
        for pat in patterns:
            if re.search(pat, con):
                p_sum += 1
        if (p_sum >= 3 and len(con) < 400) or (p_sum == 2 and len(con) < 300):
            flag = True
        # print(con, f'|{p_sum}|', '\n')
    ...
    def move_duan(self, context):
        ...
        for i, con in enumerate(context):
            # 简短段落群
            # if i < con_len/5 or i > con_len/5*4:
            #     if len(con) < 150 and (i > 0 and i < len(context) - 1 and all((len(context[j]) < 150) for j in range(i-2, i+2) if j >= 0 and j < len(context))):  # and not re.search(r'^[#\* _]*(\d+\\?\. ?|[a-z]\. )', context[j])
...
fw = open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en_clean4B.jsonl", "w", encoding="utf-8")
with open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        seq_id = item["seq_id"]
        # pattern = r'ec09cf96-eba3-40d6-a0fe-dfbf2f2bf292$'
