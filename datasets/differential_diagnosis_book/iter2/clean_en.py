...
pattern_en = [
    ...
    #09.23继续添加
    [r'(^[#\*_]*\s?([Ff]igs?(ure)?|F\s?IGS?(URE)?).*)', r'通用删除16(英):<u>\1</u>'],  # 删除开头为Figure的描述

    # 以上为通用正则库
    # ========================================================================================
    # 以下补充对此组数据清洗的特定正则
    [r'([\(（][^（）\(\)]+[，；,; ]+\d{4}[A-Za-z]?[）\)])', r'删除1:<u>\1</u>'],  # 括号内带et al\.的参考
    [r'([\(（] ?([Ff]i ?g ?\.?(ure)?|[Ss]ee|[Pp]age|[Ee]xhibit|[Pp]icture|Chapter|[Bb]ox|[Tt]able) [^（）\(\)]*[）\)])', r'删除2:<u>\1</u>'],  # 括号内图表页等
    [r'(^[\*_]*(\w{1,2}( \w{1,2})+|(\w\.?)|[\d_ ]+)[\*_]*$)', r'删除3:<u>\1</u>'],  # 无关零碎段，B、a bC、_a_ bC、C21等
    [r'(^[\*\.]*[\d _]*(?:(\d+\\?\.)|([A-Z][A-Z\-\'a-z]+[，,]? [A-Z ]{1,4}[\.，,：\(])).*(\d+(?:\(\d+\))?[：:； ]+[a-zA-Z]?\d+|[a-zA-Z]?\d+\-[a-zA-Z]?\d+[，\.]|et al\.|https?：\/\/|[， ]+p ?\d+|ed \d+，|[\.，][A-Za-z_ ]+\d{4}[，;；\._]+).*)', r'删除4:<u>\1</u>'],  # 参考文献
    [r'(^[\\\*_ ]*(\d+|[A-Z]{1,2}|SUGGESTED READING|ACKNOWLEDGMENT|Acknowledge?ments?|I. INTRODUCTION|REFERENCES?|(Selected )?References?|FURTHER READING|Further reading|5\'\-\-CACGTAAGCTATGCAGGCTT\-\-3\'|Useful websites)[\*_]*$)',  r'删除5:<u>\1</u>'],  # 参考文献标题及穿插的标题
...
class speicalProces:
    ...
    def move_ref(self, context):
        ...
        patterns = [
            ...
            r'(ed \d+[，,])|((2nd|1st|3rd|\d+th) ed)|(\([Ee]d\))',  # 版本号
...
fw = open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en_chunk_clean2.jsonl", "w", encoding="utf-8")
with open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en_chunk.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        seq_id = item["seq_id"]
        pattern = r'2c0434a8-3533-4226-a647-0c6da73f4940_60$'
