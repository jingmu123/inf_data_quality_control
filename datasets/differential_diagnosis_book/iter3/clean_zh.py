...
pattern_en_list=[
    ...
    [R'(\n|^)((?:# *参考文献\n*|参[　 ]考[　 ]文[　 ]献(?:\n-+)?)+(?:\n+\d+[　 ].*\d{4}.*)+)',r'\1删除3:<u>\2</u>'],#个例参考文献


    [r'(\n|^)([●■ ]*[^\n ][ ●■]*)(\n|$)',r'\1删除5:<u>\2</u>\3'],#删除单个字符,改
    [r'([（(]+ *(?:[参详][见考][^\n()（）]*[章节] *|参[见考](?:上文|本章)[^\n()（）]*|[选引]自[^\n()（）]*|图示[^\n()（）]*|[见续]?[图表]\d+[\–\-\—.]\d+[^\n()（）]*|[^\n()（）]*\d{4}[，,;；：:][^\n()（）]*)[)）])',r'删除6:<u>\1</u>'],#改
    ...
    [r'(\n\**|$)(便血的病因按表22-1的分类顺序讨论如下：|索 *引|吉林人民出版社.*|引自.*|SE序列 T \(a\).*|T；加权像\(a\).*)',r'\1删除个例2:<u>\2</u>'],#新
    [r'( *_[\u4e00-\u9fa5■●]+_[ ●■]*)+',r'删除15:<u>\1</u>'],#新
    [r'(。[ *]*\n+)(\**[\u4e00-\u9fa5]{1,2}。)',r'\1删除16:<u>\2</u>']#新，不完整删除
...
Line_feed_list=[
    ...
    [r'(\n|^)((?:[(（][▲0-9][）)])?[^\n#(（删图表第见一二三四五六七八九十].*[\u4e00-\u9fa5A-Za-z③0-9~] *[，\-\_、·]? *)(\n+)((?:删除\d+:<u>.*\n+|第[一-十四0-9]+.*\n+)*)( *[，\-\_\\]? *(?:[(（][▲][）)])?(?:(?:[^\n_（(#【<删图表第一二三四五六七八九十0-9])|(?:\d+[^.0-9])|(?:\d+\.\d+[%])).*)',r'\1\2 删除换行1: \5\n\4'],
...
fw = open(r"C:\Users\Administrator\PycharmProjects\untitled\new\reclean3_differential_diagnosis_book_zh.jsonl", "w", encoding="utf-8")
