...
pattern_en = [
    ...
    [r'(^[\\\*_ #]*(\d+|[A-Z]{1,2}|[a-zA-Z]?SUGGESTED READING|Suggested Reading|ACKNOWLEDGMENTS?|Acknowledge?ments?|[Rr][Ee][Fft][Ee][Rr][Ee][Nn][Cc] ?[Ee][Ss]?\.?|(Selected )?References?|FURTHER READING|Further reading( list)?|5\'\-\-CACGTAAGCTATGCAGGCTT\-\-3\'|Useful websites)[\*_]*$)',  r'删除5:<u>\1</u>'],  # 参考文献标题及穿插的标题
    ...
    [r'(^[\*_]*(Also[,，]? [Ss]ee [Cc]hapter|For more details?[,，]|Acknowledgements are|We (also )?thank|This work was|ISBN|\.com\/books|APPENDIX|Appendix).*)', r'删除12:<u>\1</u>'],  # 一些特殊无关段
    [r'(^[\\\*_]*((Data )?[Ff]rom：|Source：|Citation：).*)', r'删除13:<u>\1</u>'],  # 文献来源
    [r'([a-z]\.)( ?[\|\d]+([，, \.\-\|]+\d*)*$)', r'\1删除14:<u>\2</u>'],  # 段末无关数字
    [r'((^(?=.{0,200}$).*(Department|University).*)|(^(?=.{0,400}$).*(Depa[ri]tment of|Tel：|@[a-z]{2,10}\.com|https?).*)|(^(?=.{0,600}$).*[Ff]rom.*(https?).*))', r'删除15:<u>\1</u>'],  # 部门介绍，网址，学校
    ]
...
class speicalProces:
    ...
    def is_ref(self, con):
        flag = False
        patterns = [
            r'^[\*\.]*[\d _]*(\\?\[\d+\\?\]?|_\d+_ ?|\d+\\?[\.，][^\d]|[A-Ze][A-Z\-\'a-z]+[， ,]?[A-Z ]{1,4}[\.，,：\(])',  # 参考文献开头，序号或者人名
            r'([\s\.，,]+et al?[\.:：，,]+)',  # et al
...
with open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        seq_id = item["seq_id"]
        # pattern = r'4dc05d78-96c4-4e87-bc49-3b82e8b73d2d_98$'
