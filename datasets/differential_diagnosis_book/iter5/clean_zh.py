...
pattern_en_list=[
    ...
    [r'([，][^\n(（。，]*(?:参?见|参考|如)[图表第]\d+[\–\-\—.]\d+[^\n）)。]*。)',r'删除8:<u>\1</u>。'],#半句图片索引
    [r'([^\n(（。，；]*(?:参?见|参考|如|参阅|见)(?:[图表第]\d+|本)(?:[\–\-\—.]\d+[^\n）)。]*|[章节][^\n）)。]*))',r'删除8:<u>\1</u>'],#整句图片索引,改
    [r'(当遇到棘手的诊断问题时，表26-2所列的疾病一览表可资参考。|眼部疼痛的诊断思路与步骤是图16-1。|图示左额叶局部脑.*)',r'删除8:<u>\1</u>'],
...
Line_feed_list=[
    # [r'(\n|^)( *[见续_]?[图] *\d+([\–\-\—.]\d+)?.*)',r'\1删除4:<u>\2</u>'],#删除图片索引，改(\n|^)(\** *[见续_]?[图] *\d+(?:[\–\-\—.]\d+)?.*)(\n+\**(?:SE序列).*)?
...
fw = open(r"C:\Users\Administrator\PycharmProjects\untitled\new\reclean5_differential_diagnosis_book_zh.jsonl", "w", encoding="utf-8")
