...
pattern_en = [
    ...
    [r'((^(?=.{0,200}$).*(Department|University|New York|UK|USA).*)|(^(?=.{0,400}$).*(Depa[ri]tment of|Tel：|@[a-z]{2,10}\.com|https?).*)|(^(?=.{0,600}$).*[Ff]rom.*(https?).*))', r'删除15:<u>\1</u>'],  # 部门介绍，网址，学校
    ]
...
fw = open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en_clean6.jsonl", "w", encoding="utf-8")
with open(r"C:\Users\Administrator\Desktop\original_data\differential_diagnosis_book\differential_diagnosis_book_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        seq_id = item["seq_id"]
        # pattern = r'32de240a-1f6d-487c-a89f-f0f81ff3ba94_6$'
