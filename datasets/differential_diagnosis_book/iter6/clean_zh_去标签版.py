...
pattern_en_list=[
    [r'((?:LUNAO MRI ZHENDUA _N_ _YU JIANBIEZHENDUAN_)(?:\n{2}.+)+(?:\n{2}磁共振检查是近年来医学影像学领域中崛起的一种新的检查方法.*))',r''],
    [r'((?:\**图书在版编目\(CIP\)数据\**)(?:\n{2}.+)+(?:\n{2}责任编辑\/关静 封面设计\/黄鹤 定价：208\.00元))',r''],
    [r'((?:# 中英文名词对照索引)(?:\n{1,2}.+)+(?:\n{1,2}#### 百万电子书免费下载))',r''],
    ...
    [r'(\n|^)(#? *text\/[A-Za-z0-9]+[.]html *|#? *[a-zA-Z]+\.xhtml| *未知 *| *PDG *| *[(（]?续表[）)]? *)',r'\1'],
    ...
    [r'(当遇到棘手的诊断问题时，表26-2所列的疾病一览表可资参考。|眼部疼痛的诊断思路与步骤是图16-1。|图示左额叶局部脑.*|现按表中顺序讨论如下。|下列情况有助于胸痛的诊断与鉴别诊断：|有关弥漫性间质性肺疾病，详见第15章。|本病与单纯性肺嗜酸性粒细胞增多症（吕弗勒综合征）的鉴别可参考表5-5。)',r''],
    ...
    [r'([。] *)(_[A-Za-z0-9]_\**|[口二]\**)(\n|$)',r'\1\3'],#后缀字母
    [r'(\n|^)((?:书名|作者|页数|SS号|出版日期).*)',r'\1'],

    [r'([^\n(（。，；]*[图表第]\d+(?:[\–\-\—.]\d+可供参考|[章节])。)',r''],#表35-2可供参考
    ...
    [r'(?:([、，.][ _]*)([、，.]{1,}))',r'\1'],#改
    [r'(_[\u4e00-\u9fa5]_ *)(第[0-9一-十四]+[章节].*)',r'\2'],
    [r'(\**_[\d\—]+_)( [\d一-十四]+)(\\?[\.、])',r'\1\3'],#新
    [r'([(（]+[ 》]*[）)])',r''],#新

    [r'(\n\**|^)(便血的病因按表22-1的分类顺序讨论如下：|(?:# )?索 *引|吉林人民出版社.*|引自.*|SE序列 T \(a\).*|T；加权像\(a\).*|春 号.*|● 春.*)',r'\1'],
    [r'(\(二\)检查 )(\d)',r'\1'],
    [r'(所累|这可能与以下)(\d{3,})(及牙齿|因素有关：)',r'\1\3'],#新
    [r'(T)(\?)(加权像)',r'\1\3'],
    [r'(唯)(·-)',r'\1一']
...
Line_feed_list=[
    ...
    [r'(\n|^)((?:[(（][▲0-9^]+[）)])?(?:[^\n#(（删图表第见一二三四五六七八九十]|一般|表[现面]).*(?:[\u4e00-\u9fa5A-Za-z③0-9~]|[^u]>) *[，\-\_、·；]? *)(\n+)((?:删除\d+:<u>.*\n+|第[一-十四0-9]+.*\n+)*)( *[；，\-\_\\]? *(?:[(（][▲][）)])?(?:(?:[^\n_（(#【<删图表第一二三四五六七八九十0-9]|图像中)|(?:\d+[^\\.0-9])|(?:\d+\.\d+[%+\-×/])|(?:[(（].{5,}[）)])|(?:0\.)).*)',r'\1\2\5\n\4'],#改
    [r'(?:\n|^)(.*\([^\n()]*)(\n+)(删除\d+:<u>.*\n+)?([^\n()]*\).*)',r'\1\4\n\3'],
