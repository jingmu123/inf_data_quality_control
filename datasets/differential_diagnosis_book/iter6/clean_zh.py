...
pattern_en_list=[
    [r'((?:LUNAO MRI ZHENDUA _N_ _YU JIANBIEZHENDUAN_)(?:\n{2}.+)+(?:\n{2}磁共振检查是近年来医学影像学领域中崛起的一种新的检查方法.*))',r'删除个例:<u>\1</u>'],
    [r'((?:\**图书在版编目\(CIP\)数据\**)(?:\n{2}.+)+(?:\n{2}责任编辑\/关静 封面设计\/黄鹤 定价：208\.00元))',r'删除个例:<u>\1</u>'],
    [r'((?:# 中英文名词对照索引)(?:\n{1,2}.+)+(?:\n{1,2}#### 百万电子书免费下载))',r'删除个例:<u>\1</u>'],
    [r'((?:HIYONG RUXIAN CHAOSHENGBO JISHU PANDU JIANBIE ZHENDUAN)(?:\n{1,2}.+)+(?:\n{1,2}2005年9月))',r'删除个例:<u>\1</u>'],
    [r'(\n|^)((?:\| b\.高碘性 2\.散发性甲状腺肿.*\n{1,2}){3,})',r'\1删除个例:<u>\2</u>\n'],
    [r'((?:\**第十六章 颅脑疾病鉴别诊断\**)(?:\n{1,2}.+)+(?:\n{1,2}\**_4\\\._ 外伤后铁质沉着\**))',r'删除个例:<u>\1</u>\n'],

    [r'([# ]*(?:序[　 ]言|前[　 ]言|目[　 ]录|书名页|版权页|内容简介|丛[　 ]书.*|中山大学附属第一医院《内科疾病鉴别诊断学》编委会|主编简介|第\d+版前言|Table of Contents)(?:\n.*)*?)(?:\n#)',r'删除7:<u>\1</u>\n'],
    ...
    [r'(\n|^)((?:# *第[0-9一-十四]+章.*参考文献\n*|参考文献(?:\n-+)?)+((?:\n+[*_]*\d+[.,，、\\]?.*)|(?:\n+.*\d{4}.*)|(?:\n+第[0-9一-十四]+[章节].*)|(?:\n+(少尿或无尿|多　尿|血 尿|血红蛋白尿|脓 尿|乳 糜 尿)))*(?:(?:\n+[*_]*\d+[.,，、\\]?.*)|(?:\n+.*\d{4}.*)))',r'\1删除3:<u>\2</u>(已删除)'],#改
    [R'(\n|^)((?:# *参考文献\n*|参[　 ]考[　 ]文[　 ]献(?:\n-+)?)+(?:\n+\d+[　 ].*\d{4}.*)+)',r'\1删除3:<u>\2</u>'],
    ...
    [r'(当遇到棘手的诊断问题时，表26-2所列的疾病一览表可资参考。|眼部疼痛的诊断思路与步骤是图16-1。|图示左额叶局部脑.*|现按表中顺序讨论如下。|下列情况有助于胸痛的诊断与鉴别诊断：|有关弥漫性间质性肺疾病，详见第15章。|本病与单纯性肺嗜酸性粒细胞增多症（吕弗勒综合征）的鉴别可参考表5-5。)',r'删除8:<u>\1</u>'],


    [r'(\n|^)(\**[A-Za-z ·.。_（）()\[\]\\●■][A-Za-z0-9 ·.。_（）()\[\]\\●■]*\**|\**[0-9]+[A-Za-z0-9 ·。_（）()\[\]\\●■]*\**)(\n|$)',r'\1删除10:<u>\2</u>\3'],#字母符号句子，改
    # [r'(\n|^)(附[:：].*|主审[:：/].*|注[:：].*)',r'\1删除11:<u>\2</u>'],

    [r'(\n|^)(【[^\n】\\\]\} ]*)(\\\]|\})?(\n|$)',r'\1\2补充符号1：】\4'],
    [r'(\n|^)(\**【[^\n】 ]*)( [^\n】]{10,})(\n|$)',r'\1\2补充符号2：】\3\4'],
    [r'(\n|^)(\\\[)?([^\n【]*】.*)(\n|$)',r'\1补充符号3：【\3\4'],

    [r'([。] *)(_[A-Za-z0-9]_\**|[口二]\**)(\n|$)',r'\1删除12:<u>\2</u>\3'],#后缀字母
    [r'(\n|^)((?:书名|作者|页数|SS号|出版日期).*)',r'\1删除13:<u>\2</u>'],

    [r'([^\n(（。，；]*[图表第]\d+(?:[\–\-\—.]\d+可供参考|[章节])。)',r'删除14:<u>\1</u>'],#表35-2可供参考

    [r'(\n|^)(( *_[\u4e00-\u9fa5■●]+_[ ●■]*)+)(\n|$)',r'\1删除15:<u>\2</u>\3'],
    [r'(。[ *]*\n+)(\**[\u4e00-\u9fa5]{1,2}。)',r'\1删除16:<u>\2</u>'],#不完整删除

    [r'(\n|^)( *\d{1,3} *- *\d{1,3}.*)',r'\1删除18:<u>\2</u>'],
    [r'(\n|^)(\**_?\d+[ \\]*\._? *)([,，、])',r'\1\2删除19:<u>\3</u>'],
    # [r'((、){2,}|(?:[ _]*(，)[ _]*){2,}|(?:[ _]*(\.)[ _]*){2,})',r'\2\3\4删除20:<u>\2\3\4</u>'],
    [r'(?:([、，.][ _]*)([、，.]{1,}))',r'\1删除20:<u>\2</u>'],#改
    [r'(_[\u4e00-\u9fa5]_ *)(第[0-9一-十四]+[章节].*)',r'删除21:<u>\1</u>\2'],
    [r'(\**_[\d\—]+_)( [\d一-十四]+)(\\?[\.、])',r'\1删除23:<u>\2</u>\3'],#新
    [r'([(（]+[ 》]*[）)])',r'删除24:<u>\1</u>'],#新

    [r'(\n\**|^)(便血的病因按表22-1的分类顺序讨论如下：|(?:# )?索 *引|吉林人民出版社.*|引自.*|SE序列 T \(a\).*|T；加权像\(a\).*|春 号.*|● 春.*)',r'\1删除个例2:<u>\2</u>'],
    [r'(\(二\)检查 )(\d)',r'\1删除个例3:<u>\2</u>'],
    [r'(所累|这可能与以下)(\d{3,})(及牙齿|因素有关：)',r'\1删除个例4:<u>\2</u>\3'],#新
...
def clean_text(context, lang):
    ...
    for pattern_item in pattern_list:
        # print(pattern_item[0])
        context = re.sub(pattern_item[0], pattern_item[1], context)

    for pattern_item in Line_feed_list:
