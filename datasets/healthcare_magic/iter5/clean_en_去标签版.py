...
pattern_en_list=[
    [r'( *(?:[Hh][Ii](?: [A-Za-z0-9]{2,15})?[ \.!,]?|[Hh][Ee][Ll]{2}[Oo](?: [A-Za-z0-9]{2,15})?[ \.!,]?| ?and ?| ?[Tt][Oo] ?| ?[Dd][Ee][Aa][Rr] ?[A-Z][a-z]*\.[a-z]*[ \.!,]?| ?[Ww][Ee][Ll]{1,2} ?[Cc][Oo][Mm][Ee] ?| ?[Tt][Hh][Aa][Nn][Kk][Ss]? ?| ?[Ff][Oo][Rr] [Yy][Oo][Uu][Rr] [Qq][Uu][Ee][Rr][Yy] ?){2,}[^\n.?!]*(?:service|HCM|hcm|[Mm][Aa][Gg][Ii][Cc]|[Hh][Ee][Aa][Ll][Tt][Hh] ?[Cc][Aa][Rr][Ee]|site|[Ff][Oo][Rr][Uu][Mm])[\.?!]?)',r''],#新
    [r'(\. If you wish to post them in future, bring it to my attention \("Attn: Dr\. Amit Rajput"\)|\(about like the 0 in this print\)|\(If the answer has helped you, please indicate this\)|\**Question\**: image attached *| *I hope you will find this useful Regards take care *|Wathch the video and read the pdf file.*|David Dzsizan@gmail\.com *|RegardsBinu|Thank you for choosing HealthcareMagic\.|For a personalised comprehensive evaluation.*|[Tt](?:hanks?|HANKS?) (?:you|YOU|for) (?:from|FROM|[Cc]hoosing) (?:HCM|hcm) *\.?|(?:[Hh]i[ \.]?|[Hh]ello[ \.]?)?Welcome to (?:Healthcare|HEALTHCARE)(?: Magic *\.?| MAGIC *\.?)?)',r''],
    [r'((?:\:\-?\(+)|(?:\:\-?\)))',r''],

    [r'(?:([\.!\?,])|([A-Za-z]))( *(?:Kind [Rr]egards|(?:[Tt]ake [Cc]are |[Ww]ith |[B]est )?[Rr][Ee][Gg][Aa][Rr][Dd][Ss] *[\.!,]?(?: *[Dd][Rr])?|[Dd][Rr]|Best wishes|(?:Take Care\.)?Best|Yours truly|Truly yours|Hennie|Kim|Cindy|Sandi Brown|Aisha|(?:Many *)?[Tt]hanks!?(?: and [Rr]egards[\.,]?)?|Best of luck|[Tt]ake care[\.!]?|Thank(?:ing)? you(?: *[Dd][Rr])?|Cheers & Godspeed!|X{3,}|(?:[Ww]ish(?:ing)?(?: you| him)?)? *(?:(?:good)? health|[Aa]ll the best|a nice weekend)!?(?: *[Dd][Rr])?|"?[Ww]ith.{1,20}wishes[",]?Dr|(?:[Oo][Kk])? *than(?:Q|l?k u)|Get well soon|Thanks in advance)+(?:[ .&\-,]+[A-Z][^\n ]*)* *(?:\([^\(\)\n]+\))? ?)(\n|$)',r'.\2\4'],

    [r'([\.\?!])((?:(?:[^\n\.!?])+(?:[Dd][Rr]\.))*(?:[^\n,\.!?])*)((?:[Hh]ttps?[：:][/／]{2}(?:www[.．])?|www[.．])[a-zA-Z0-9@／/-]+(?:[.．／/][a-zA-Z0-9@／/_\-]{2,})+.*)',r'\1'],#改

    [r'(you can ask a direct question to me on this forum, following the below link.*)',r''],
    [r'([\.!\?])([^\n.]*(?:(?:[Dd ][Rr]\.)[^\n.]*)*(?:YYYY@YYYY|@[A-Za-z0-9]+\.com| [Ll][Ii][Nn][Kk] *[:：]|@ *[Ww]{2,4}\.).*)',r'\1'],#改
    [r'([\.!\?])( *(?:Please suggest|, take care|[A-Za-z]|Randi Ginder|thanks dr.dhara dhara.shah84@yahoo\.in) *)(\n|$)',r'\1\3'],
    [r'([,，] *(?:[Tt]ake care).{1,50})(\n|$)',r'. \2'],
    [r'(\*\*Answer\*\*: Brief Answer: *(?:Thank you using|Please rate).*)',r''],
    [r'([\.!\?,])( *(?:(?:With )?Best Wishes|Dr|Regards)(?:Dr)?.*(?:http:\/+)?(?:www\/)?(?:[ \.]+W+)+ *)(\n|$)', r'.\3'],
    [r'([A-Za-z])((?:[Rr][Ee][Gg][Aa][Rr][Dd][Ss]).{0,30})(\n|$)',r'\1.\3'],

    [r'([:：])( *(?:(?:[A-Za-z\'][A-Za-z，,\'’\.\-]*|[&/]|[Aa][Nn][Dd]) +)+Response)',r'\1'],#改
    [r'([,\.!?])( *(?:[Ss][Ii][Nn][Cc][Ee][Rr][Ee][Ll][Yy] *,.{0,100}|help pls *))(\n|$)',r'\1\3'],#新
    [r'([.!?])( *[^\nA-Za-z0-9)\.!"”\'?/ ] *)(\n|$)',r'\1\3'],#新
    [r'([^0-9][.!?])( *[0-9] *)(\n|$)',r'\1\3'],#11.5新

    [r'(?:([\.!\?,])|([A-Za-z]))([Rr][Ee][Gg][Aa][Rr][Dd][Ss][ ,]*[Dd][Rr][ .]*[^\n.]*\.?)(\n|$)',r'.\2\4'],
...
# with open(r"C:\Users\Administrator\PycharmProjects\untitled\healthcare_magic\test.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        context = re.sub(r"(\*\*(Question|Answer)\*\*:)", r"\n\n\1", context)
