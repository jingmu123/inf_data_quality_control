...
def clean_text(context, lang):
    ...
    while i < len(new_context):
        if re.search(r'(^[\*#]{0,4}([Ff]igs?(ure)?|F\s?IGS?(URE)?)[\*#]{0,4}\s?\d+.{0,4}\|删除)',new_context[i]):
            new_context[i] = re.sub(r'(^[\*#]{0,4}([Ff]igs?(ure)?|F\s?IGS?(URE)?)[\*#]{0,4}\s?\d+.*)',r'删除1:<u>\1</u>',new_context[i])
        print(new_context[i])
        i += 1
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean2_oxford.jsonl", "w", encoding="utf-8")
