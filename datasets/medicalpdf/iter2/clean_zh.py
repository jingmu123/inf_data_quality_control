...
class speicalProces:
    ...
    def step8_repair_headline_locate(self,text):
        ...
        origin_lis, done_lis, locate_lis, final_return = [], [], [], ''
        raw = text['attr']["raw_info"]
        for part in raw:
            locate = part['full_blocks']
            x_center, y_center = int((locate[0] + locate[2]) / 2), int((locate[1] + locate[3]) / 2)

            origin_lis.append([x_center, y_center, part['block_text']])
        ...
        for index, single in enumerate(origin_lis):
            ...
            sum_half_x = origin_lis[temple_index][1] / 2 + origin_lis[index][1] / 2
            if x_def < sum_half_x:
