...
[
    r'(\(\s?\b([Ff]igure[s]?|Section|diagram|Appendix|Box|[Ss]ource[s]?|[Ff]igs?|p)\b[^\)\n]*\))',
    r''
],
...
class speicalProces:
        ...
        if raw_info != []:
            ...
            for raw in raw_info:
                ...
                if self.is_page_foot(full_blocks, img_box,first_left,first_right):
                    ...
                    # 使用正则表达式替换匹配的文本
                    item['text'] = re.sub(rf"{block_text}", fr'页边侧栏删除：<u>{block_text}</u>', item['text'])
            if replace_num > 10:
                item['text'] = '(本页删除)本页中有超过10个边角的判断怀疑是目录图片页，本页删除：' + item['text']
        return item
    ...
    def step2_delete_photopage(self, item):
        ...
        if sum(all_block_area) < img_area * 0.2:
    ...
    def step9_complete_sequence_number(self, text):
        ...
        while state:
            ...
            if result:
                ...
                last_num = result.group(4)

                while int(last_num) == int(fir_num) + 1:
                ...
                for index, part in enumerate(target_list):
                    ...
                    text = re.sub(part, str(int(fir_num) + index + 1) + '.' + replace_part + '缺少序号', text)
...
def process_line(items):
    item = json.loads(items.strip())
    context = clean_text(item)
...
if __name__ == "__main__":
    ...
    fw = open("C:/Program Files/lk/projects/pdf/medicalpdf/medicalpdf_preformat_en_7.jsonl", "w", encoding="utf-8")
    with open("C:/Program Files/lk/projects/pdf/medicalpdf/medicalpdf_preformat_en.jsonl", "r", encoding="utf-8") as fs:
        for items in tqdm(fs.readlines()):
            item = json.loads(items.strip())
            # if item["seq_id"] == "ee93c9c8-51ad-4ba8-b083-2d8b0342ce2f":
                # print(item)
            context = item
            # lang = item["lang"]
