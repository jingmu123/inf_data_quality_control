...
[
    r'(\(\s?\b([Ff]igure[s]?|Section|diagram|Appendix|Box|[Ss]ource[s]?|[Ff]ig|p)\b[^\)\n]*\))',
    r''
],
[
    # r'([^\n\.]*\bFigs?(ure)?s?\b\.?[\s* ](\d+)?\.?[^\n]*)',
...
class speicalProces:
    ...
    def step3_more_linefeed(self, context):
        ...
        while index < len(context):
            ...
            if index > 0:
                if stripped_item and context[index - 1][-1] not in ['.','!','?'] and (stripped_item[0].islower() or stripped_item[0] in ["(", "[", ")", "]", "."]):
                    """
...
fw = open("C:/pycharm/orc识别pdf清洗数据/pdf/clean_json/medicalpdf_preformat_en_3.jsonl", "w", encoding="utf-8")
with open("C:/pycharm/orc识别pdf清洗数据/pdf/clean_json/medicalpdf_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    for items in tqdm(fs.readlines()):
        item = json.loads(items.strip())
        # if item["seq_id"] == "9886b8b5-d3bd-4eb9-adab-2bbd80d1eff1":
        # print(item)
        context = item
        # lang = item["lang"]
