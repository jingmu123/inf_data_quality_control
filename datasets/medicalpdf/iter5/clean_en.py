...
class speicalProces:
    ...
    def is_page_foot(self, least_bbox, img_box, first_left, first_right):
        ...
        # 检测左侧边角
            ...
            return True
        # 上边角会遇到标题这个问题，要不要解决？
        elif least_bbox[3] - img_box[1] <= img_box[3] * 0.06:
            return True
        else:
            return False
    def is_fig(self,block_text):
        if re.search('^(Figs?(ure)?s?|FIGURE)',block_text):
            # print(block_text)
            return True
        return False
    ...
    def step3_2_more_linefeed_duan(self,context):
        ...
        while index < len(context):
            ...
            if index >= 0:
                ...
                # elif "#" not in stripped_item and re.search(r'[^A-Z\.?!:：”"\)]$', stripped_item.strip()):
                ...
                #         if index + 1 < len(context) and "#" not in context[index + 1] and self.is_merge_ngram(
                #                 last_line_of_current, first_line_of_next) and not re.search(r'^([Ff]ig|\d+\.)',
                #                                                                             first_line_of_next):
                #             # 合并到下一个 item
...
def clean_text(context):
    ...
    # 多余换行
    context = sp.step3_1_more_linefeed_duannei(context)
...
fw = open("C:/pycharm/orc识别pdf清洗数据/pdf/clean_json/medicalpdf_preformat_en_5.jsonl", "w", encoding="utf-8")
with open("C:/pycharm/orc识别pdf清洗数据/pdf/clean_json/medicalpdf_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    for items in tqdm(fs.readlines()):
        item = json.loads(items.strip())
        # if item["seq_id"] == "9d1a9e42-5b07-4e0a-a926-788a5f971d34":
        # print(item)
        context = item
        # lang = item["lang"]
