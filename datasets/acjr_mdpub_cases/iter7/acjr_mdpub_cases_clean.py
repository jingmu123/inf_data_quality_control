...
class clean_pattern:
    ...
    def delete_page_ending(self, context):
        ...
        # 避免重复加标签，特征最好合并为1-2条，当段保留一条，当段删除一条。
        ending_starts = [
            [r'^[#\*]{0,4}\s?(Re[tf]e?r?ences?：?|\|? *REFERENCES?：? *\|?|Funding ?(Sources|Statement|and Disclosure|program)?|Polls on Public|Ethic(s|al) ([Cc]ommittee ?)?([Aa]pproval：?)?( and informed consent)?|Author[s\'’ ]*([Cc]ontributions?：?|[Ss]tatement|[Dd]eclaration)|Acknowledge?men[t1]：?|(Acknowledgements )?Cc?onflic?ts? of [Ii]nterest( and source of funding| [Ss]tatement)?|Source of (Support|Funding)|(Financial )?[Dd]isclosure|(Disclosure |Ethics )?Statement( of Ethics)?|Declaration of (Figure[s\'’ ；]*|Tables )Authenticity|Competing [Ii]nterest( Nil\.)?|Declarations?( of( competing)? interest)?|Patient informed consent|(Department and )?Institution [Ww]here [Ww]ork [Ww]as ([Dd]one|[Pp]erformed)|CONFLICT OF INTEREST：?|COMPETING INTERESTS|PATIENT CONSENT|TAKE HOME MESSAGES?|AUTHOR[S\'’ ]*CONTRIBUTIONS?|Authorship|ACKNOWLEDGE?MENTS?|Main Institute for the Case|Institutional review board statement|REFENRCES：|Informed consent( and patient detail)?|Disclosure of financial arrangement|Ethical Permission|CONSENT|Role of funding source|FUNDING)s?[：\.]?[#\*]{0,4}\s{0,}($|\n)'],
...
class speicalProces:
    ...
    def move_ref_confusion(self, context):
        ...
        if match:
            ...
            if n_text in "Conclusions" or n_text in "CONCLUSIONS" or n_text in "Acknowledgments" or n_text in "Acknowledgements":
                context = re.sub(
                    r'([a-zA-Z，\d\-\.\)\]])(\n\n(Re[tf]e?r?ences?：?|REFERENCES?：?\n\n)[\W\w]*?)(\n\n)((\\?\-?[a-zA-Z\(]{2})[\W\w]*?)(\n\n\d+\\?\.)',
                    r'\1|拼接引用正文|\n\n\5\2\7', context)
                return context
...
with open(r"C:/Program Files/lk/projects/pdf/acjr_mdpub_cases/acjr_mdpub_cases_preformat.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        if seq_id in wuguan_list:
            continue
        if seq_id == "a6d87f3d-10b6-439b-abc5-8739e04b05d7":
