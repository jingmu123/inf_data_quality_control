...
pattern_en = [
    ...
    [r'(\(\s?([Ff]igs?(ure)?|F\s?IGS?(URE)?|[Tt]able|[Ss]ee|For more|panel|http|www|NCT\d+|NO\.|version|p\.)s?[\s\.:]?[^\(\)]*\))', r''],  # 1. 这些固定的词语紧贴左括号
    ...
    [r'(^(Full?-text |Copyright|DOI：|(Name of |[\dl]? ?)?Depa[ri]tment |Dr\. |organizations， or those of the publisher|New York Chiropractic and|Publisher[\'s ]*note：|Corresponding author：|Internal Medicine (.*)?Department|Cc?onflicts? of interest|National Center of Cancer|[A-Z][a-z]+ of Medicine，|Heart Attack\.|start of Hajj in 2008).*)', r'删除2:<u>\1</u>'],
    [r'(.*(Adress|E-mail|Phone( number| No\.)|Received|Accepted|Editor)[：:].*)', r'删除3:<u>\1</u>'],
    ...
    [r'(^((We|The authors) (would like to|gratefully) (acknowledge|thank)|Joana Vilaca\\\*1，|\\\*Servigo de Pediatria|Video \d+\.|Clemenceau Medical Center|The authors?( gratefully)? thank|There are no conflicts|No financial support of|O Am J Case Rep|1\\. van Riet EES， Hoes AW，|age size：|Abdomen venoes Axial|FUFFWO\\_|Series：|We are grateful to |1\\. Carlson JA：|Abstract of this work).*)', r'删除12:<u>\1</u>'],
...
class clean_pattern:
    ...
    def delete_page_ending(self, context):
        ...
        # 避免重复加标签，特征最好合并为1-2条，当段保留一条，当段删除一条。
        ending_starts = [
            [r'^[#\*]{0,4}\s?(Re[tf]e?r?ences?：?|\|? *REFERENCES?：? *\|?|Funding ?([Ss]ources|[Ss]tatement|and Disclosure|program)?|Polls on Public|Ethic(s|al) ([Cc]ommittee ?)?([Aa]pproval：?)?( and informed consent)?|Author[s\'’ ]*([Cc]ontributions?：?|[Ss]tatement|[Dd]eclaration)|Acknowledge?men[t1]：?|(Acknowledgements )?Cc?onflic?ts? [oi]f [Ii]nterest( and source of funding| [Ss]tatement)?|Source of (Support|Fund(ing)?)|(Financial )?[Dd]isclosure|(Disclosure |Ethics )?Statement( of [Ee]thics)?|Declaration of (Figure[s\'’ ；]*|Tables )Authenticity|Competing [Ii]nterest( Nil\.)?|Declarations?( of( competing| conflicting)? interest)?|Patient informed consent|(Department and )?Institution [Ww]here [Ww]ork [Ww]as ([Dd]one|[Pp]erformed)|CONFLICT OF INTEREST：?|COMPETING INTERESTS|PATIENT CONSENT|TAKE HOME MESSAGES?|AUTHOR[S\'’ ]*CONTRIBUTIONS?|Authorship|ACKNOWLEDGE?MENTS?|Main Institute for the Case|Institutional review board statement|REFENRCES：|Informed consent( and patient detail)?|Disclosure of financial arrangement|Ethical Permission|CONSENT|Role of funding source|FUNDING|Disclaimer|Data [Aa]vailability( Statement)?|Acknowledgement，Source\(s\) of Support None\.|Disclosures and freedom of investigation|Author[s\'’ ]*information and contribution|No relevant conflict of interests\.|Ethics and Dissemination|Disclosure of interest)s?[：\.]?[#\*]{0,4}\s{0,}($|\n)'],
    ...
    def delete_page_middle(self, context):
        ...
        start_to_end = [
            # 样例
            [r'(^[\\ #]*(Copyright @))', r'(^[\\ #]*([A-Z][A-Za-z]+( and)?( [A-Z][A-Za-z]+)?|Background：)[\\ #]*$)|(^.{250,})|(^[a-z]{1,3}.{72,}\.$)|(^([A-Z][\w ()]{50,}\. ?){2,})', 0],
            [r'(^[\\ #]*(DOI：))', r'(^[\\ #]*([A-Z][A-Za-z]+( and)?( [A-Z][A-Za-z]+)?|Background：)[\\ #]*$)|(^.{250,})|(^[a-z]{1,3}.{72,}\.$)|(^([A-Z][\w ()]{50,}\. ?){2,})', 0],
            [r'(^[\\ #]*(Full?-text ))', r'(^[\\ #]*([A-Z][A-Za-z]+( and)?( [A-Z][A-Za-z]+)?|Background：)[\\ #]*$)|(^.{250,})|(^[a-z]{1,3}.{72,}\.$)|(^([A-Z][\w ()]{50,}\. ?){2,})', 0],
            [r'(^[\\ #]*(Author[s\'’ ]*[Aa]ddress：|[\dIl]?Corresponding [Aa]uthors?：|Received：))', r'(^[\\ #]*([A-Z][A-Za-z]+( and)?( [A-Z][A-Za-z]+)?|Background：)[\\ #]*$)|(^.{250,})|(^[a-z]{1,3}.{72,}\.$)|(^([A-Z][\w ()]{50,}\. ?){2,})', 0],
        ]
    ...
    def move_ref_confusion(self, context):
        ...
        if match:
            ...
            # if n_text in "Conclusions" or n_text in "CONCLUSIONS" or n_text in "Acknowledgments" or n_text in "Acknowledgements" or n_text in "Disclosures":
                context = re.sub(r'([a-zA-Z，\d\-\.\)\]])(\n\n(Re[tf]e?r?ences?：?|REFERENCES?：?\n\n)[\W\w]*?)(\n\n)((\\?\-?[a-zA-Z\(]{2})[\W\w]*?)(\n\n\d+\\?\.)', r'\1|拼接引用正文|\n\n\5\2\7', context)
                return context
            elif re.search(r'(\d{4}[;；： \.]+\d+(\(\d+\))?[:：； ]*[\d\-]+)|(htt[tp]s?[：:]?\/)|([^\[]\d+(\(\w+\))?[:：； ]*\d+[\-]\d+)|(20|1[6-9])\d{2}|(^Table \d)|([Vv]ol\.| ed[\.；]|Volume|\.h[ti]ml)|(\d+(\(\w+\))?[:：； ]+pp?\d+\.)', l_text) or re.search(r'(\d{4}[;；： \.]+\d+(\(\d+\))?[:：； ]*[\d\-]+)|(htt[tp]s?[：:]?\/)|([^\[]\d+(\(\w+\))?[:：； ]*\d+[\-]\d+)|(20|1[6-9])\d{2}|(^Table \d)|([Vv]ol\.| ed[\.；]|Volume|\.h[ti]ml|(\d+(\(\w+\))?[:：； ]+pp?\d+\.))', n_text) or len(n_text)<25:
...
# fw = open(r"C:/Program Files/lk/projects/pdf/acjr_mdpub_cases/acjr_mdpub_cases_preformat_clean4.jsonl", "w", encoding="utf-8")
