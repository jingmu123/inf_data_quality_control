...
pattern_en = [
    ...
    [r'(^(Full?-text |Copyright|DOI：|[\dl]? ?Department |Dr\. ).*)', r'删除2:<u>\1</u>'],
    [r'(.*(Adress|E-mail|Phone( number| No\.)|Received|Accepted|Editor)[：:].*)', r'删除3:<u>\1</u>'],
    [r'(^(\(\w+\) ?\(\w+\)|\d\w( \d\w)*|[\d\-]+：|[A-Z]( \w)*)$)', r'删除4:<u>\1</u>'],
...
class clean_pattern:
    ...
    def delete_page_ending(self, context):
        ...
        # 避免重复加标签，特征最好合并为1-2条，当段保留一条，当段删除一条。
        ending_starts = [
            [r'^[#\*]{0,4}\s?(References?：?|REFERENCES?：?|Funding( Sources| Statement| and Disclosure)?|Polls on Public|Ethics (committee )?[Aa]pproval|Author[s\' ]*Contribution|Acknowledge?ment|Conflicts? of [Ii]nterest|Source of (Support|Funding)|(Financial )?[Dd]isclosure|Statement)s?[#\*]{0,4}\s{0,}($|\n)'],
    ...
    def delete_page_middle(self, context):
        ...
        start_to_end = [
            # 样例
            [r'(^[\\ #]*(Copyright @))', r'(^[\\ #]*([A-Z][A-Za-z]+( [A-Z][A-Za-z]+)?)[\\ #]*$)|(.{250,})', 0],
            [r'(^[\\ #]*(DOI：))', r'(^[\\ #]*([A-Z][A-Za-z]+( [A-Z][A-Za-z]+)?)[\\ #]*$)|(.{250,})', 0],
            [r'(^[\\ #]*(Full?-text ))', r'(^[\\ #]*([A-Z][A-Za-z]+( [A-Z][A-Za-z]+)?)[\\ #]*$)|(.{250,})', 0],
        ]
...
fw = open(r"C:/Program Files/lk/projects/pdf/acjr_mdpub_cases/acjr_mdpub_cases_preformat_clean1.jsonl", "w", encoding="utf-8")
