...
pattern_en = [
    ...
    [r'((\\)?\[[\d\s,\\，\–\-—\_]*(\\)?\])', r'通用删除6(英):<u>\1</u>'],  # 带有方括号的数字引用
...
pattern_en_list=[
    ...
    [r'(\n|^)([^\n_].*(?:[DM]r\.|[A-Za-z]) *_{3}[_ ]*)(\n+)([_ ]*\(?_{3}[_ ]*\)? *[,，\.!?a-zA-Z])',r'\1\2 删除换行13 \4'],
    # [r'(\n|^)( ?(?:[^\n•●©# ]{1,} +){5,}[^\n]+[A-Z] *)(\n+)( *[A-Z][^\n\. ]*[^:：\-] +(?:(?:[^\n•●©:： ]{1,} +){5,}.*)|(?:[^\d\.\n:：]{2,}\. *(?:\n|$)))',r'\1\2 删除换行15 \4'],  # 大写字母和大写字母
    [r'(\n|^)( ?(?:[^\n•●© ]{1,} +){5,}[^\n]+[A-Z] *)(\n+)( *[A-Z][^\n\. ]*[^:：\-] +(?:(?:(?:[^\n•●©:： ]{1,} +){5,}.*)|(?:[^\d\.\n:：]{2,}\. *(?:\n|$)))|(?: *\(.{3,}\)))',r'\1\2 删除换行15 \4'],# 大写字母和大写字母 改。。。。。。。。。。。。。。。
...
fw = open(r"C:\Users\Administrator\PycharmProjects\untitled\mimic_iv_discharge_preformat\reclean4_mimic_iv_discharge.jsonl", "w", encoding="utf-8")
