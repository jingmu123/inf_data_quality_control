...
pattern_list_en = [
    ...
    [r'(\()\b(Table(s?)|table(s?)|TABLE|Figure(s?)|figure(s?)|Section|diagram|Appendix|Box|(S|s)ource(s?))\b:?\s*(\d+)?(\.?).*?(\))', ''],
    ...
    [r'[^\.\n]*at\s*\.$',''],
    [r'[^\.\n]*Images\s*shown[^\.\n]*',''],
    [r'[^\n]*(Manufactured for)|(AUTHORS)|(External link)|(Classes and members)|(LINKS)[^\n]*',''],
