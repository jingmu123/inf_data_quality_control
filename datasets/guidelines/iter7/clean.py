...
pattern_list_en = [
    ...
    [r'\(go\s*to\s*(R|r)esults\s*in\s*(S|s)ection\s*\d+.*?\)', ''],
    ['(\(\d\s*References)?(\(See)?.*\.\.\.\sread more\s*\)\.', ''],  # 1 References See...sread more.
    [r'(\((see|See)\s*(Table|table|Figure|figure|Section)?.*?\))', ''],
    [r'(\()\b(Table|table|TABLE|Figure|figure|Section|diagram|Appendix|Box|(S|s)ource(s?))\b:?\s*(\d+)?(\.?).*?(\))', ''],
    [r'[^\.]*\b(\b(T|t)able\b|TABLE|(F|f)igure|(S|s)ection|diagram|(A|a)ppendix|Box)\b[^\.\n]*', ''],
    ...
    [r'[^\.\n]*\d+-\d+-\d+[^\.\n]*',''],#去掉NDC 0065-0023-15
    [r'[^\d\(]*pictured[^\)]*',''],
    # [r'(\b\d{1,2}\b)?([,\.])?\b\d{1,2}\b$',''], #去除句末无关数字引用，如：# TEAM BRIEFING 3,4
    [r'[^\(\.\n]*see\s*illustration[^\)\.\n]*',''],
    [r'[^\.\n]*thank[^\.\n]*',''],
    [r'#\s*Disclaimer',''],
    [r'[^\.\n]*WikiDoc[^\.\n]*',''],
...
def clean_text(context, lang):
    ...
    for item in context.split(split_token):
        ...
        for i in range(len(item)-1,-1,-1):
            if item[i].startswith("#") or item[i].startswith("*"):
                item.pop()
            else:
                break
        item = ("\n".join(item)).strip()
