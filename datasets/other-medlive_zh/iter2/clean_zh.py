...
pattern_list_zh = [
    ...
    [r'((\*\*)?[(（] *(?:(?:详|流程)?见|配|如|视频|网络|截)*[图表] *[A-Za-z0-9一二三四五六七八九十]*[^(（）)\n]*?[）)](\*\*)?)', r'删除7：<u>\1</u>'],  # 文中的图表索引
    [r'(?<=\n)(△?.{0,8}(?:资料|[截配制])图.{0,10})(?=\n|）|\))',r'删除26：<u>\1</u>'],#各式截图类的单行删除
    [r'(?:。| |）|，|:)([^ 。,，\n(（）)]*?[如见]?下?[图表] *(?:[A-Za-z0-9一二三四五六七八九十]|所示)+[^。,，\n(（）)]*)(?:。|\n)',r'删除14：<u>\1</u>'],#文中出现的有关见图见表类的小句子的删除


    [r'(?:\n|^)(\**[(（]?(第一作者|(本文)?编辑|(本文)?作者|撰稿人|资料图|联系.{0,3}|.{0,3}日期|.{0,3}邮箱|电话|手机|审批.{0,3}|致谢|支?持?单位|点评专家)[：:│].*\**)',r'删除8：<u>\1</u>'],
    [r'(?<=\n)( * *责编 *[:：]?.*)|(?<=^)( * *责编 *[:：]?.*)',r'删除9：<u>\1\2</u>'],#单独一行的责编删除

    [r'(<sup>(<span>)? *\[?.*?\]? *(<\/span>)?<\/sup>)',r'删除3：<u>\1</u>'],
    [r'(\\?[\[［(][0-9０-９ ]+(?:[-—–－,，、][0-9０-９ ]+)*\\?[)］\]])',r'删除5：<u>\1</u>'],#文中的数字索引：\[5\]，\[16-17\]，\[7,8\]，［９］
    [r'(?:\n|^)(\**(?:\\\*|…|参考文献 *略。?|↓|▼|摄图网|\**.*请勿转载.*\**|大会主持|。|致谢和参考文献（略）|.*内容仅限.*|.*以上观点系.*|本[期文]案例来自.*)+)(?=\n|$)',r'删除10：<u>\1</u>'],#单独一行\*或...的删除
    ...
    [r'(?<=\n)((.{4,7})(?:(?:(?:作|进行).{0,4}报告)|(开场致辞)))',r'删除24：<u>\1</u>'],
    [r'(?<=\n)([A-Za-z]+号 *[A-Za-z0-9]+ *有效期 *\d{4}-\d{2}-\d{2}.*(\n\n)?(资料过期，视同作废。)?)',r'删除25：<u>\1</u>'],
    [r'(?<=\n)((?:\**.*专家组\**)\n*\( *按姓氏笔画排序 *\)(?:.|\n)*?)(?:\n *\n *\n|$)',r'删除27：<u>\1</u>'],
    [r'(.*欢迎(评论区)?留言.*)',r'删除28：<u>\1</u>'],
    [r'(?<=)((?:[\u4e00-\u9fa5]{2,3} )+译)(?:\n)',r'删除29：<u>\1</u>'],
    [r'([(（][A-Za-z .]+\d{4}[,，;]\d+(?:\(\d+\))?[:：,，]\d+[-–—]\d+[^\u4e00-\u9fa5\n]+[)）])',r'删除30：<u>\1</u>'],

    #特殊case
    [r'(\**(?:(?:欲知后事如何，请听下周日分解！|今天的医疗圈|发生了哪些与你有关的大事？|更新、更全的医学动态|更新、更全的医学动态|3分钟一网打尽|新旧版本更换期)\n*)+\**)',r'删除31：<u>\1</u>']
...
sp=speicalProces()
fw = open(r'C:\Users\Administrator\PycharmProjects\untitled\other-medlive_zh_preformat\reclean2_other-medlive_zh_label.jsonl', 'a', encoding='utf-8')
