...
pattern_list_zh = [
    ...
    [r'(?:^[  ]*|\n[  ]*)((\*\*)?△*.*(截?[图表]+片?)[来源自略]+[:：]?.*(\*\*)?)',r'删除20：<u>\1</u>'],#图源类的删除整行删除
    [r'(?:\n|^)([  ]*\**△?附?(?:本文)?[图表]注? *[A-Za-z0-9一二三四五六七八九十:：片|]+.*\**)(?=\n|$)', r'删除6：<u>\1</u>'],  # 单独一行的图表索引
    [r'((\*\*)?[(（] *(?:(?:详|流程)?见|配|如|视频|网络|截|点击查看大?|作者供)*[图表] *[^现柔][A-Za-z0-9一二三四五六七八九十]*[^(（）)\n]*?[）)](\*\*)?)', r'删除7：<u>\1</u>'],  # 文中的图表索引
    [r'(?<=\n)(△?.{0,15}(?:资料|[截配制]|流程)图.{0,10})(?=\n|$)',r'删除26：<u>\1</u>'],#各式截图类的单行删除
    [r'(。| |）|，|:)([^ 。,，\n(（）)]*?[如见]?下?[图表] *(?:[A-Za-z0-9一二三四五六七八九十]|所示)+[^。,，\n(（）)]*)(?:。|\n)',r'\1删除14：<u>\2</u>'],#文中出现的有关见图见表类的小句子的删除
    [r'([(（][^\n()（）]+)([,，][图表] *[A-Za-z0-9一二三四五六七八九十:：]*)([)）])',r'\1删除34：<u>\2</u>\3'],


    [r'(?:\n|^)(\**[(（]? *((?:第一|本文|文章)?作者(?:介绍|信息|贡献|简介)?|(本文)?编辑|翻译|撰稿人|分享人|撰文|统筹|策划|绘制|编译|利益冲突|审校|.{0,2}译者|资料图|Editor|联系.{0,3}|.{0,3}日期|.*邮箱|电话|手机|.{0,2}审批.{0,3}|致谢|支?持?单位|点评专家|执笔| *CRC number *| *Approved date *)[：:│丨/].*\**)',r'删除8：<u>\1</u>'],
    [r'(?<=\n)( * *责编 *[:：]?.*)|(?<=^)( * *责编 *[:：]?.*)',r'删除9：<u>\1\2</u>'],#单独一行的责编删除

    [r'[^0-9m](<sup>(<span>)? *\[?[^A-Za-z]*?\]? *(<\/span>)?<\/sup>)',r'删除3：<u>\1</u>'],
    [r'([0-9/m])(<sup>(?:<span>)? *(\[?[^A-Za-z]*?\]?) *(?:<\/span>)?<\/sup>)',r'\1\3删除3：<u>\2</u>'],



    [r'(\\?[\[［(][0-9０-９ ]+(?:[-—–－,，、][0-9０-９ ]+)*\\?[)］\]])',r'删除5：<u>\1</u>'],#文中的数字索引：\[5\]，\[16-17\]，\[7,8\]，［９］
    [r'(?:\n|^)(\** *(?:\\\*|…|.*参考文献.*略.*|↓|—|▼|摄图网|\**.*(?:请勿|不得)转载.*\**|大会主持|。|致谢和参考文献（略）|.*内容仅[限供].*|.*以上观点系.*|本[期文]案[例件]来自.*|.{0,10}作者供图)+)(?=\n|$)',r'删除10：<u>\1</u>'],#单独一行\*或...的删除
