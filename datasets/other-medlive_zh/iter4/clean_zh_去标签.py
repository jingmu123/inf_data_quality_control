...
pattern_list_zh = [
    ...
    [r'(([(（])?\[([^\n[\]]*?)<sup><img><\/sup>\]\(https?:\/\/[a-zA-Z0-9-]+\/\d*\))',r'\2\3'],#多余链接和格式的删除

    [r'(?:^[  ]*|\n[  ]*|[(（]| )((\*\*)?△*来[源自][:：].*(\*\*)?)',r''],#来源类的整行删除
    [r'(?:^[  ]*|\n[  ]*)((\*\*)?△*.{0,15}(截?见?[图表]+片?)[来源自略0-9A-Za-z。]+[:：]?.*(\*\*)?)',r''],#图源类的删除整行删除
    [r'(?:\n|^)([  ]*\**△?附?(?:本文)?[图表]注?解? *[A-Za-z0-9一二三四五六七八九十:：片|]+.*\**)(?=\n|$)', r''],  # 单独一行的图表索引
    [r'((\*\*)?[(（] *(?:(?:详|流程)?见|配|如?下?|视频|网络|截|点击查看大?|作者供)*[图表] *[A-Za-z0-9一二三四五六七八九十]*[^(（）)\n]{0,5}[）)](\*\*)?)', r''],  # 文中带括号的图表索引
    [r'(?<=\n)(△?.{0,15}(?:资料|[截配制]|流程|网络)图.{0,10})(?=\n|$)',r''],#各式截图类的单行删除
    [r'(。| |）|，|:)([^ 。,，\n(（）)]*?[如见]?下?[图表] *(?:[A-Za-z0-9一二三四五六七八九十]|所示)+[^。,，\n(（）)]*)(?:。|\n)',r'\1'],#文中出现的有关见图见表类的小句子的删除1
    [r'(。| |）|，|:)([^ 。,，\n(（）)]*?[如见]下?[图表] *)(?:。|\n|“|”|；|;)',r'\1'],#文中出现的有关见图见表类的小句子的删除2
    [r'([(（][^\n()（）]+)([,，;；][图表] *[A-Za-z0-9一二三四五六七八九十:：]*)([)）])',r'\1\3'],#括号内有部分图片内容，只删除部分


    [r'(?:\n|^)(\**[(（]? *((?:第一|本文|文章)?作者(?:介绍|信息|贡献|简介)?|(本文)?编辑|翻译|撰稿人|分享人|撰文|统筹|策划|绘制|编译自?|利益冲突|审校.{0,2}|.{0,2}译者|资料图|Editor|联系.{0,3}|.{0,3}日期|.*邮箱|电话|手机|.{0,2}审批.{0,3}|致谢|支?持?单位|点评专家|执笔| *CRC number *| *Approved date *|病例(?:总结|提供者)|公众号(?:[Ii][Dd])？)[：:│丨/ ].*\**)',r''],
    [r'(?<=\n)( * *责编 *[:：]?.*)|(?<=^)( * *责编 *[:：]?.*)',r''],#单独一行的责编删除

    [r'[\u4e00-\u9fa5](<sup>(<span>)? *\[?[^A-Za-z]*?\]? *(<\/span>)?<\/sup>)',r''],
    [r'([^\u4e00-\u9fa5])(<su[pb]>(?:<span>)? *(\[?[^A-Za-z]*?\]?) *(?:<\/span>)?<\/su[pb]>)',r'\1\3'],



    [r'(\\?[\[［(][0-9０-９ ]+(?:[-—–⁃－,，、][0-9０-９ ]+)*\\?[)］\]])',r''],#文中的数字索引：\[5\]，\[16-17\]，\[7,8\]，［９］
    [r'([\u00B3-\u00BA\u2070-\u2079\u00B2]+(?:[-—–⁃⁻－,，、][\u00B3-\u00BA\u2070-\u2079\u00B2]+)*)',r''],
    ...
    [r'(?:\n|^)( *\**(?:（?摘要号 *[:：]|(?:英文|中文|原文?)?标题 *[:：]|文献链接 *[:：]|文章来源 *[:：]|延伸阅读 *[:：]|.{0,8}摘自\**[：:]|.{0,8}选自|.{0,8}整理自).*\**)',r''],#ee1a37d2-a04e-4e8a-80ca-5a16de7af2d6
    ...
    [r'[\u4e00-\u9fa5](?<!分|为|图|表|日|在|是|或|于|一|和)([0-9０-９]{1,2}[-—，,][0-9０-９]{1,2})。',r''],
    [r'((\** *[\u4e00-\u9fa5]{0,8} *[:：] *\**)?\[[^\]\n]*\n\]\(.*\))',r''],
    [r'([⑴-⑾])(?:。)',r''],
    [r'(?:\n|^)( *[\u4e00-\u9fa5]{1,5} *\|[ A-Za-z.]+)(?=\n|$)',r'']
