...
pattern_list_zh = [
    ...
    [r"(\[ ?\"? ?(\d{1,3} ?[-–,\.]+ ?)+\d? *\*?)([。\.,，\u4e00-\u9fa5a-zA-Z\(（] ?)", r"\3"],
    # [r"([\u4e00-\u9fa5] [^\u4e00-\u9fa5\d~。？！]*?)( ?\d+ *?)(。| \.)", r"\1删除5:<u>\2</u>\3"],
    # [r"([\u4e00-\u9fa5])(\s*?\.\s*?\d+\s*?)([\(\u4e00-\u9fa5。\n])", r"\1删除6:<u>\2</u>\3"],
    # [r'([\u4e00-\u9fa5》)])(\s?\d+\s?)(。|\s\.)', r"\1删除7:<u>\2</u>\3"],

    # # 参考应用的复杂格式：字母(Ia,\nb)
    [r'(\( ?[IⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫa-zA-Z]? ?[a-zA-Z]? ?, ?[a-zA-Z] ?\) ?)[。\.]', r""],
    ...
    [r'(\s?(👍|▶|●|©|\(?↑\)?|†|¶|║|§|∧|™|■|◆|♕|ኤ|♦|ż|❏|□|✓|✔|❍|▲|⭐|◎|●|۞|\s__\s|￥|∩|‡|♂|¿|☞|⌒|�|\$|￼|भ|£|♀|œ|¾|ä|も|へ|す|➪|Ø|「|\s…\s)\s?)', r''],
    ...
    [r'(?<=^)([\(（] ?(引 *自 *)?[\u4e00-\u9fa5]( *[\u4e00-\u9fa5] *){1,2} ?[）\)]?)(?=\n|$)|(?<=\n)([\(（] ?(引 *自 *)?[\u4e00-\u9fa5]( *[\u4e00-\u9fa5] *){1,2} ?[）\)]?)(?=\n|$)',r''],#（人名）
    [r'([\?？,，:：;；。.~\-—])(( ?[\?？,，:：;；。~] ?){2,})',r'\1'],
    [r'((?:https?:\/\/)?(?:www\.)[a-zA-Z0-9-]+(?:\.[a-zA-Z]{2,})+(?:\/[^ ]*)?)',r''],
    [r'([\(（][^\u4e00-\u9fa5\nA-Za-z%π//]*?( ?[–\-= ，,)] ?\d+){2,}[^\u4e00-\u9fa5\nA-Za-z%π//]*?[）\)])',r''],#( 2 53-3-3 )
    [r'([(（][^\n()（）\\]*?参见[^\n()（）\\]*?[）)])',r''],
