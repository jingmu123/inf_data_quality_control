...
pattern_list_zh = [
    # fig cite
    # [r"([见和]图 ?\d+ ?[。？！\.]?)", r"删除1：<u>\1</u>"],
    # 参考引用：数字类型，加入空格做泛化：[1,2 - 3, 4, 5]
    [r'(\[ ?(\d{1,3} ?[-–,，\.]+ ?)+\d+ ?\] ?\*?)', r"删除2:<u>\1</u>"],
    # 参考补充，一些缺少]的，以符号作为右边界
    [r"(\[ ?\"? ?(\d{1,3} ?[-–,\.]+ ?)+\d? *\*?)([。\.,，\u4e00-\u9fa5a-zA-Z\(（] ?)", r"删除4:<u>\1</u>\3"],
    # [r"([\u4e00-\u9fa5] [^\u4e00-\u9fa5\d~。？！]*?)( ?\d+ *?)(。| \.)", r"\1删除5:<u>\2</u>\3"],
    # [r"([\u4e00-\u9fa5])(\s*?\.\s*?\d+\s*?)([\(\u4e00-\u9fa5。\n])", r"\1删除6:<u>\2</u>\3"],
    # [r'([\u4e00-\u9fa5》)])(\s?\d+\s?)(。|\s\.)', r"\1删除7:<u>\2</u>\3"],

    # # 参考应用的复杂格式：字母(Ia,\nb)
    [r'(\( ?[IⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫa-zA-Z]? ?[a-zA-Z]? ?, ?[a-zA-Z] ?\) ?)[。\.]', r"删除3:<u>\1</u>"],
    ...
    [r'(?<=^)([\(（] ?(引 *自 *)?[\u4e00-\u9fa5]( *[\u4e00-\u9fa5] *){1,2} ?[）\)]?)(?=\n|$)|(?<=\n)([\(（] ?(引 *自 *)?[\u4e00-\u9fa5]( *[\u4e00-\u9fa5] *){1,2} ?[）\)]?)(?=\n|$)',r'删除40:<u>\1\4</u>'],#（人名）
    [r'([\?？,，:：;；。.~\-—])(( ?[\?？,，:：;；。~] ?){2,})',r'\1删除多余符号：<u>\2</u>'],
    [r'((?:https?:\/\/)?(?:www\.)[a-zA-Z0-9-]+(?:\.[a-zA-Z]{2,})+(?:\/[^ ]*)?)',r'删除网址：<u>\1</u>'],
    ...
    [r'(^[。\n].*?见图[a-zA-Z\d]+.*?)[。\n]', r"删除11:<u>\1</u>"],
    # 图引用2
    [r'([\(（] ?图 ?[a-zA-Z\d] ?[和与,，、]? ?图 ?[a-zA-Z\d] ?[\)）])', r"删除12:<u>\1</u>"],
    # 图引用3
    ...
    [r'(^[Aa]dd ?[sS]?)', r"删除20:<u>\1</u>"],
    [r'(\.[Aa]dd[sS]\.?$)', r"删除21:<u>\1</u>"],
    # [r"(\s?[\xAC00-\xD7AF]\s?)",r"删除201:<u>\1</u>"],
    [r"([^\u4e00-\u9fa5。\n\]]*)(小[狗街].{0,3}[科研葡公开设讨社花究所课系心,\d a-zA-Z]+ ?)([^\u4e00-\u9fa5。\n]?)",
     r"\1删除23:<u>\2</u>\3"],
...
class speicalProces:
    ...
    def step3_rm_noise_dig_alpha(self, context):
        ...
        import numpy as np
        # 有汉字
        str_lis=[r'[\u4e00-\u9fa5]',r'[\da-zA-Z]+ *\.',r'#',r'=',r'\+',r'/',r'—']
        for str in str_lis:
            if len(re.findall(str, context)) > 0:
                return context
            else:
                continue
    ...
    def step4_rm_cite(self,context_raw):
        ...
        # print(cite_tag)
        if sum(cite_tag) >= 3 and len(context) < 150:
    ...
    def is_complete(self, last_sentence, curr_sentence):
        ...
        conjunction=[r'\d{1,4}\.[^\d]',r'((?:\(|（) ?[0-9-一二三四五六七八九十]+ ?(?:）|\)))',r'([-一二三四五六七八九十]{1,2}[\.、])[^\d]',r'##','[A-E]\.','[【[〖] ?[\u4e00-\u9fa5]{2,4} ?(\d* )?[〗\]】]','图 *\d']
    ...
    def step9_remove_en_noise(self,text):
        ...
        common_elements = ["Na", "Mg", "Al", "Si", "Cl", "Ca", "CH", "Ag", "Hg", "Cu", "mol", "nm", "mm", "cm", "mg","kg","lg",
                           "Hz", "Hb", "xOy", "OH", "H 2 O","H 2 0","SO","RNA","DNA","Fe","x ","Pa","gK"]
        ...
        origin_lis.sort(key=lambda x: x.lstrip().rstrip().lower())
        ...
        for word in origin_lis:
            ...
            if word_copy.isupper()  and len(word_copy)<=7:
                continue
    ...
    def step10_Irrelevant_long_text(self,context):
        ...
        key_words=['教材编写','前言','作者简介','作者介绍','版权所有','主编介绍','参考文献','姓氏笔画排序']
        ...
        if suspected:
            ...
            for paragraph in context_lis:
                ...
                if output["labels"][0] != 'Text content' and output['scores'][0]>=0.5:
