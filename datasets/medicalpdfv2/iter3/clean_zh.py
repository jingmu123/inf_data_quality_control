...
pattern_list_zh = [
    ...
    [r'(\s?(👍|▶|●|©|®|\(?↑\)?|†|¶|║|§|∧|™|■|◆|♕|ኤ|♦|ż|❏|□|✓|✔|❍|▲|⭐|◎|●|۞|\s__\s|￥|∩|‡|♂|¿|☞|⌒|�|\$|♀|œ|¾|ä|&|「|\s…\s)\s?)', r'删除28:<u>\1</u>'],
...
class speicalProces:
    ...
    def is_complete(self, last_sentence, curr_sentence):
        ...
        conjunction=[r'\d{1,4}\.[^\d]',r'((?:\(|（) ?[0-9-一二三四五六七八九十]+ ?(?:）|\)))',r'([-一二三四五六七八九十]{1,2}[\.、])[^\d]',r'##','[A-E]\.','删除']
        No_closing_remarks=["、", "对", "与", "和", "及", "其", "但", "且", "乃", "~", "～","的", "由","于", "致","—","-",",","，"]
        for word in conjunction:
            if re.findall(word, last_sentence.replace(' ', '')[:4]) == []:
                last_sentence_switch=True
            else:
                last_sentence_switch=False
                break
        for word in conjunction:
            if re.findall(word, curr_sentence.replace(' ', '')[:4]) == []:
        ...
        # elif "#" in last_sentence or "#" in curr_sentence:
            ...
            return True
        elif len(re.findall(r'^[^#\d\(（](?:.*?[a-zA-Z])([a-zA-Z\d]$)(?<!g)', last_sentence)) > 0 and curr_sentence_switch or len(re.findall(r'^[a-zA-Z\d][^\.][^\.]', curr_sentence)) > 0 and last_sentence_switch:
            # print(111)
            return False
        ...
        while re.findall(r'(^[^#\d\n\(（第一二三四五六七八九十].*?[\u4e00-\u9fa5,，"”’:：)）])( *\n{1,} *)(["”’:：,，학\u4e00-\u9fa5])',context, flags=re.MULTILINE):
            ...
            if target!=copy_target:
                for single in target:
                    if '删除换行' in single[0] or '删除换行' in single[2]:
                        continue
                    if len(single[0]) <= 9 or len(single[2]) <= 9:
                        # if single not in copy_target:
                        #     copy_target.append(single)
                        continue
    ...
    def step9_remove_en_noise(self,text):
        ...
        # print('原', suspect_lis)
        for word in suspect_lis:
            word_copy = word.lstrip().rstrip()
            if word_copy.isupper() and ' ' not in word_copy and len(word_copy)<5:
                continue
    ...
    def step10_Irrelevant_long_text(self,context):
        ...
        if '教材编写' in context or '前言' in context or '作者简介' in context or '作者介绍' in context:
            # print(context)
...
def clean_text(context, lang, sp):
    ...
    # 分解处理
    result = []
    # context = re.sub(r'(^[^#\d\n\(（一二三四五六七八九十].*?[\u4e00-\u9fa5])( *\n{1,} *)([\u4e00-\u9fa5])', r'\1\3', context,flags=re.MULTILINE)
...
with open(r'F:\zeroshots\medicalpdfv2_0724_preformat_zh.jsonl', "r", encoding="utf-8") as file:
    for item in tqdm(file.readlines()):
        ...
        with open(r'F:\zeroshots\reclean6_medicalpdfv2__preformat_zh.jsonl', 'a', encoding='utf-8') as f:
            f.write(item+'\n')
