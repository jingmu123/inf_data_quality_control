...
class speicalProces:
    ...
    def is_merge_duannei(self, text, next_text):
        # 定义一个介词列表
        preposition_list = ['of', 'in', 'and', 'or', 'not', 'the', 'a', 'any', 'for', 'is', 'The', 'A', 'with', '&', 'And', 'their','his','her','at']
        if re.search(r'\s\d+\.$', text): #
            return True
        if re.search(r'^#{0,3}\s?([\d+A-Z][\.,\s]?){1,5}$', text.strip()) and re.search(r'^#{0,3}\s?[A-Z]',next_text.strip()):  # 匹配段落中只有序号且下一段是大写开头的情况 都可以带#
            return True
...
# fw = open("C:/pycharm/orc识别pdf清洗数据/pdf/clean_json/reclean3_medicalpdfv2_0724_en.jsonl", "w", encoding="utf-8")
with open("C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\medicalpdfv2_0724_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    for items in tqdm(fs.readlines()):
        item = json.loads(items.strip())
        if item["seq_id"] == "7646144c-1d7c-48a9-a876-944cbb45dec3":
        # print(item)
            context = item
