        ...
        if raw_info != []:
            ...
            for index, raw in enumerate(raw_info):
                ...
                if self.is_page_foot(full_blocks, img_box, first_left, first_right):
                    ...
                    # 使用正则表达式替换匹配的文本
                    try:
                        item['text'] = re.sub(rf'{escaped_least_text}', r'', item['text'])
            ...
            if index >= 0:
                if index > 0 and stripped_item and context[index - 1][-1] not in ['.', '!', '?', '|',':'] and not re.search('#',stripped_item) and (
                    ...
                    if not re.search(r'#', context[index - 1]):
                        # 合并到前一个 item
                        context[index - 1] = context[index - 1].rstrip() + " " + item.lstrip().lstrip()
                        # 删除当前 item
                        del context[index]
                        # 继续检查当前索引位置的元素
                        index = index - 1
                        ...
                        if index - 2 and len(re.split(r'\n', context[index - 2])) > 1 and context[index - 2][-1] not in ['.', '!', '?']:
                            ...
                            if self.is_merge_ngram(last_line_of_previous, first_line_of_current):
                                # 合并context[index - 2]
                                context[index - 2] = context[index - 2].rstrip() + " " + item.lstrip().lstrip()
                                # 删除当前 item
                                del context[index]
                                # 继续检查当前索引位置的元素
                                index = index - 1
                ...
                #     continue
                    ...
                    if index + 1 < len(context) and not re.search(r'^[Ff]ig', context[
                        index + 1].lstrip()):
                        # 合并下一个 item
                        context[index] = item.rstrip() + " " + re.sub("^#{0,3}","",context[index + 1].lstrip().lstrip())
                        # 删除下一个 item
                        del context[index + 1]
                        # 不增加 index, 继续检查当前索引位置的元素
                        index = index - 1
                    ...
                    if match:
                        ...
                        # 合并下一个 item
                        context[index] = part1 + " " + stripped_item + " " + part2
                        # 删除下一个 item
                        del context[index + 1]
                        # 不增加 index, 继续检查当前索引位置的元素
                        index = index - 1
...
with open("C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\medicalpdfv2_0724_preformat_en.jsonl", "r", encoding="utf-8") as fs:
    for items in tqdm(fs.readlines()):
        item = json.loads(items.strip())
        # if item["seq_id"] == "e1f46df3-a837-468e-825c-fd7f488cb540":
        # print(item)
        context = item
