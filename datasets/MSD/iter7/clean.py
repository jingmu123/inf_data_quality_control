...
pattern_list_en = [
    ...
    [r'(表格)?(Table)? \$\(document\).ready.*?\} \}\); \} \}\);', r'\n'],
    [r'(\$\(document\))?\.\s?ready.*?console', r'\n'],
    [r'\b([^\d]+)(\s+\1){1,2}\b',r'\1'],
    # [r'(\(\s?s?S?ee[^\.]*)?\.?\(?[^\.]*[\.]{0,3} read more [^A-Z\.]*(\.\))?', '\n'],
    # 83 86固定顺序除去read more后面是右括号，匹配左括号
    [r'\([^\(\)]*?read more[^\)\(]{0,100}[\s\.]{0,3}\)',''],
    [r'\([^\(\)]*?(\([^\(\)]{0,100}\)[^\(\)]*?){0,2}read more[^\(\)]*?(\([^\(\)]{0,100}\)[^\(\)]*?){0,2}[\s\.]{0,3}\)',''],
    [r'\(\s?[sS]ee[^\.\(\)]*(\([^\(\)]{0,100}\)[^\.\(\)]*){0,3}[\.\s]{0,3}\)', ''],
    [r'([^\.]*?\d+\.){0,3}[^\.]*?([Ss]ee.{0,300})?(\.\.\.)? read more[^\.\(\)?!]*(\(.{0,350}?\)[^\.\(!?]{0,400}){0,5}[\.!?\)]{0,3}(\d+[^.?!]*.){0,2}',r'\n'],
    # Common.TooltipReadMore分解,顺序不能变
    # Common.TooltipReadMore后面有右括号，去匹配他的左括号中间允许成对括号出现  这种情况删除掉的文本前后内容不用替换成换行
    [r'\([^\(\)]{0,300}[\.]{0,3}?\s?Common.TooltipReadMore([^\.!?\(\)]{0,50})?[\s\.]{0,3}\)',''],
    [r'\([^\(\)]{0,300}(\([^\)\(]{0,200}\)[^\(\)]{0,200}){0,3}[\.]{0,3}?\s?Common.TooltipReadMore[\s\.]{0,3}\)',''],
    [r'[^\.]*?[\.]{0,3}?\s?Common.TooltipReadMore[^\.]*\.?\)?', r'\n'],

    [r'\(Adapted from[^\(]{0,200}(\([^\)]{0,200}\)[^\(]{0,200}){0,3}\)',r'\n'],
    [r'(Table[^\.]{0,300})?(,)?(\s+Diagnosis reference 1.\s+)?Adapted from [^\.\n]*\.(.{0,300},\s?\d{4}\.)?(\s+Copyright 1992, American Medical Association.\s+)?( doi: 10.1161/01.cir.102.10.1193. )?',r'\n'],
    ...
    [r'[^\n]*et al\s?:[^\n]*', r'\n'],
    [r'\n\s+?[a-z][^\.]*\.', r'\n'],
    # 之前的规则把前面可能出现的common给删掉了被替换成了换行符,所以从换行符开始删除到句号
    [r'[^\n]*?TooltipReadMore[^\.]*\.',r'\n'],
    # 特殊情况 去重的正则抓不到它
...
class speicalProces:
    ...
    def step4_remove_duplicates(self, text, lang):
        ...
        for sentence in sentences:
            old_sentence = sentence
            sentence = re.sub(r'\s+','',sentence).strip()  # 去除空格和换行符加入set列表对比
...
def post_process(context):
    ...
    context = re.sub(r"\b([^\d]+)(\s+\1){1,2}\b",r'\1',context,flags=re.I)
