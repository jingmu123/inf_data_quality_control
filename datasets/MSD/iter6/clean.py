...
pattern_list_en = [
    ...
    # 83 84固定顺序除去read more后面是右括号，匹配左括号
    [r'\([^\(\)]*?read more[^\)]{0,100}[\s\.]{0,3}\)',''],
    [r'\([^\(\)]*?(\([^\(\)]{0,100}\)[^\(\)]*?){0,2}read more[\s\.]{0,3}\)',''],
    [r'\(\s?[sS]ee[^\.\(\)]*(\([^\(\)]{0,100}\)[^\.\(\)]*){0,3}[\)\.\s]{0,3}', ''],
    [r'([^\.]*?\d+\.){0,3}[^\.]*?([Ss]ee.{0,300})?(\.\.\.)? read more[^\.\(\)?!]*(\(.{0,350}?\)[^\.\(!?]{0,350}){0,5}[\.!?\)]{0,3}(\d+[^.?!]*.){0,2}',r'\n'],
...
fw = open("MSD_preformat_6_en.jsonl", "w", encoding="utf-8")
