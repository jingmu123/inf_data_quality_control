...
pattern_en = [
    ...
    [r'(\([^\(\)]*([Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|[sS]ee\s|For more|http|www|NCT\d+|N[oO]\.|Participant \d+|Provider \d+|software|version)s?[\s\.:\d][^\(\)]*\))', r'通用删除1(英):<u>\1</u>'],  # 最广泛的形式从左括号匹配到右括号
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean1_journal-pile.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\journal-pile_preformat.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        # if item["seq_id"] == "01ab1a5d-35d6-424b-aa6f-9010abff64ff":
        context = item["text"]
        lang = item["lang"]
        title = item["title"]
