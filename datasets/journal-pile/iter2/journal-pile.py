...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean2_journal-pile.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\journal-pile_preformat.jsonl", "r", encoding="utf-8") as fs:
    lines = fs.readlines()
    sampled_lines = random.sample(lines, 300)
    for items in tqdm(sampled_lines):
        item = json.loads(items.strip())
        # if item["seq_id"] == "dc06d464-cdcf-4b9d-b906-0f9daba201fc":
        context = item["text"]
        lang = item["lang"]
        title = item["title"]
