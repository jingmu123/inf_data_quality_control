...
pattern_en = [
    ...
    [r'(\.)((Table|Fig|Figure|Video)(\.)?\s?\d[A-Z].*)',r'\.删除43:<u>\2</u>'],      # 处于句子的句末的Table/Fig描述
    [r'(^(E-mail|Twitter|Published|iThenticate screening|Financial source|Clinical Trial number \(ReBEC\)|Acknowledgments):[.\n]*)',r'删除44:<u>\1</u>'],   # E-mail:开头的句子
...
class clean_pattern:
    ...
    def delete_page_start(self, context):
        ...
        end_pattern = [
            [r'(^[#\s]*(Abstract|ABSTRACT|INTRODUCTION)：?\s.{0,50}$)', 0],
            [r'(^[#\s]*(1\.\s?)?(Introduction)：?\s.{0,50}$)', 0],
...
def clean_text(context, lang):
    ...
    for i, item in enumerate(context):
        ...
        if lang == "en":
            ...
            if re.search(r'^(Ethics Statement|Role of the funding source|Code availability|PROM Questionnaire|Sample Availability)[.\n]*', item):
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean7_journal-pile.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\journal-pile_preformat.jsonl", "r",
    ...
    for items in tqdm(sampled_lines):
        ...
        # if item["seq_id"] == "16687a51-56a0-43c7-8993-1e2e61b15d8f":
        context = item["text"]
        if len(context) > 5000:
            lang = item["lang"]
            title = item["title"]
