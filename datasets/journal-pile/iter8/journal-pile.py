...
pattern_en = [
    ...
    [r'(.*(equally to this work|contributed equally).*)',r'删除46:<u>\1</u>'],    #  ...对此贡献对等
    [r'(.*All authors (participated|have read).*)',r'删除48:<u>\1</u>'],   #All authors participated 所有的作者都参加了...
    [r'([^_])((Table|Fig)(\.)?\s?\d\s?$)',r'\1删除49:<u>\2</u>'],
    [r'(.*(This research received no external funding|revised the manuscript|[Pp]ublished online:|All authors read and approved the final manuscript).*)',r'删除50:<u>\1</u>'],   # 这项研究没有得到外部资助
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean8_journal-pile.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\journal-pile_preformat.jsonl", "r",
    ...
    for items in tqdm(sampled_lines):
        ...
        # if item["seq_id"] == "8239500f-b782-4500-8995-938672543ab2":
        context = item["text"]
        lang = item["lang"]
        title = item["title"]
