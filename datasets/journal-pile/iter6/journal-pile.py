...
pattern_en = [
    ...
    [
        r'((\^)?(\\)?\([^\[]{,50}([,;\.\s]{0,}\[[^\[\]]*(@|#|Supplementary material|Reporting Summary|SUMMARY|data not shown|www|[Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|[sS]ee)[^\[\]]*\]){1,}[^\]]{,50}(\\)?\)(\^)?)',
        r''],  # 带有圆括号 里面有一个特殊符号@、#
    [
        r'((\^)?(\\)?\[[^\[]{,50}([,;\.\s]{0,}\[[^\[\]]*(@|#|Supplementary material|Reporting Summary|SUMMARY|data not shown|www|[Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|[sS]ee)[^\[\]]*\]){1,}[^\]]{,50}(\\)?\](\^)?)',
        r''],  # 带有方括号 里面有一个特殊符号@、#
    [
        r'(\^)?((\\)?[\[\(])?([,;\.\s]{0,}\[[^\[\]]*(@|#|Supplementary material|Reporting Summary|SUMMARY|data not shown||[Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|[sS]ee)[^\[\]]*\]){1,}((\\)?[\]\)])?(\^)?',
        r''],  # 带有方括号 里面有一个特殊符号@、#
    ...
    [r'(.*These authors contributed equally.*)', r'删除32:<u>\1</u>'],  # 固定表述
    [r'(\[\^\d+\])', r''],  # 无关数字
    [r'(\(:\s?[①②③④⑤⑥⑦⑧\-⑨⑩\s,\.]{1,}\))', r''],
    ...
    [r'(^(The authors?|We are).*(thanks?|grateful|acknowledge|indebted|appreciates?).*)', r'删除26:<u>\1</u>'],  # 作者感谢...的支持之类的描述
    [r'(^(Informed )?[cC]onsent:.*)', r'删除27:<u>\1</u>'],  # 知情同意、同意 ...
    [r'^(\([A-Z]*\)|[#]{1,}|Click here for additional data file.)$', r'删除28:<u>\1</u>'],  # 单行只有(A-Z)、多个#、点击查看文件
    [r'(.{20,}Supporting information.*)', r'删除29:<u>\1</u>'],
    # ...支持信息... 如果Supporting information在段首则对下面的段落全部删除  否则只删除这一行
    #
    ...
    [r'(.*(conflicts? of|competing) interest[^$]*)', r'删除37:<u>\1</u>'],  # ...利益冲突... 一般在句中只能针对这句去删除
    ...
    [r'(\.)((Table|Fig|Figure)(\.)?\s?\d[A-Z].*)',r'\.删除43:<u>\2</u>'],      # 处于句子的句末的Table/Fig描述
    ...
    [r'(.*This research received no external funding.*)',r'删除50:<u>\1</u>'],   # 这项研究没有得到外部资助

    [r'(.*revised the manuscript.*)',r'删除51:<u>\1</u>'],
    [r'(.*[Pp]ublished online:.*)',r'删除52:<u>\1</u>'],
...
pattern_zh = [
    ...
    [r'(?<![\dm\s])(\s{0,}<sup>(<a>)?[\d\s\–—,\(\)\[\]]{1,20}(</a>)?</sup>)', r'通用删除11(中):<u>\1</u>'],
    # 特殊数字  排除可能出现的次幂情况
...
def clean_text(context, lang):
    ...
    for i, item in enumerate(context):
        ...
        if lang == "en":
            ...
            if re.search(r'^(Ethics Statement|Role of the funding source|Code availability)[.\n]*', item):
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean6_journal-pile.jsonl", "w", encoding="utf-8")
