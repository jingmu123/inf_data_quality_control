...
pattern_en = [
    ...
    [
        r'((\^)?(\\)?\([^\[]{,50}([,;\.\s]{0,}\[[^\[\]]*(@|#|Supplementary material|Reporting Summary|SUMMARY|data not shown)[^\[\]]*\]){1,}[^\]]{,50}(\\)?\)(\^)?)',
        r''],  # 带有圆括号 里面有一个特殊符号@、#
    [
        r'((\^)?(\\)?\[[^\[]{,50}([,;\.\s]{0,}\[[^\[\]]*(@|#|Supplementary material|Reporting Summary|SUMMARY|data not shown)[^\[\]]*\]){1,}[^\]]{,50}(\\)?\](\^)?)',
        r''],  # 带有方括号 里面有一个特殊符号@、#
    [
        r'(\^)?((\\)?[\[\(])?([,;\.\s]{0,}\[[^\[\]]*(@|#|Supplementary material|Reporting Summary|SUMMARY|data not shown)[^\[\]]*\]){1,}((\\)?[\]\)])?(\^)?',
        r''],  # 带有方括号 里面有一个特殊符号@、#
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean4_journal-pile.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\journal-pile_preformat.jsonl", "r",
    ...
    for items in tqdm(sampled_lines):
        ...
        # if item["seq_id"] == "002a052c-a40f-4581-8920-ca647bec8466":
        context = item["text"]
        if len(context) > 2000:
            lang = item["lang"]
            title = item["title"]
