...
pattern_en = [
    ...
    [r'(\(\s?([Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|[Ss]ee|For more|panel|http|www|NCT\d+|NO\.|version|p\.|Supplementary [mM]aterial)s?[\s\.:\d][^\(\)]*\))', r''],  # 1. 这些固定的词语紧贴左括号
    ...
    [r'(^(This|The)\s.{0,50} (was|is) (supported|funded)[^$]*)', r'删除38:<u>\1</u>'],  # 这项研究得到了...的支持
...
def clean_text(context, lang):
    ...
    for i, item in enumerate(context):
        ...
        if lang == "en":
            ...
            if re.search(r'^(Ethics Statement|Role of the funding source)[.\n]*', item):
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean5_journal-pile.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\journal-pile_preformat.jsonl", "r",
    ...
    for items in tqdm(sampled_lines):
        ...
        # if item["seq_id"] == "17888dc3-1500-47a1-a50e-19b503ff386a":
        context = item["text"]
        if len(context) > 5000:
            lang = item["lang"]
            title = item["title"]
