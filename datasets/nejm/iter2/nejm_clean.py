...
pattern_list = [
    ...
    [r'(\(\s?([Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|Details|Appendix|Funded|Section|epoch|Panels?|[sS]ee|for example, see)s?[\s\.][^\(\)]*\))',r''],   # 固定格式  带有（）的图片表格描述 附录描述 协议描述
    [r'([^\d\sm]+\s?)(<sup>(<a>)?[\d\s\-—,]{1,20}(</a>)?</sup>)',r'\1'],   # 特殊格式的数字引用删除
    ...
    [r'(\([^\(\)]*(www\.|NEJM\.org|https?:|versions?\s?\d|opens? in new tab)[^\(\)]*\))',r''],    # 匹配带括号且有网址特征的句子
    [r'(([^\.]*\d+\s?\.){0,3}[^\.]*at NEJM.org\.)',r''],   # 带有NEJM.org描述但是没有括号的句子 删除8只能在删除7后面
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean2_nejm_preformat.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\nejm_preformat.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        # if item["seq_id"] == "fa85ceca-a086-4751-8228-c83e74432419":
        context = item["text"]
        lang = item["lang"]
