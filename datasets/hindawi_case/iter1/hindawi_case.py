...
pattern_en = [
    ...
    [r'(\(([Ff]igs?(ure)?|F\s?IGS?(URE)?)[^\(\)]*(\([^\(\)]{0,5}\)[^\(\)]*){0,}\))',r'删除1:<u>\1</u>'],
    [r'(^((\s){0,}(\([\da-z]\)|©)(\s){0,}){1,}($|\n))',r''],
    [r'',r''] , # 特殊符号
        ...
        while index < len(context):
            ...
            for line_feed_rule in Line_feed_rules:
                ...
                # 如果规则长度为 1，只需要匹配当前行
                    ...
                    if index + 1 < len(context) and re.search(current_line_rule, stripped_item, re.I):
                        # 合并当前段和下一段
                        context[index] = item.rstrip() + " " + context[index + 1].lstrip()
                        # 删除下一段
                        del context[index + 1]
...
class speicalProces:
    ...
    def step0_common_clean(self,context,cp,lang):
        ...
        Line_feed_rules = [
            [r'^([Ff]igs?(ure)?|F\s?IGS?(URE)?|Table|TABLE)\s?\d+\s?$']
        ]
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean1_hindawi_case.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\hindawi_case_preformat.jsonl", "r", encoding="utf-8") as fs:
    ...
    for items in tqdm(lines):
        ...
        # if item["seq_id"] == "33d157df-0eff-4db4-9a4d-0c0ae43d73b1":
        context = item["text"]
        lang = item["lang"]
        title = item["title"]
