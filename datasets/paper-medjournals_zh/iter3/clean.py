...
pattern_zh_list=[
    ...
    [r'(\n|^|\.|。)([\–\-\—]*(?:原文出?处?[:：]|摘译?自|N Engl|分类号[:：]|志谢[:：]?|基金[\u4E00-\u9FFF]{0,5}[:：]|[\u4E00-\u9FFF]{0,2}作者[:：]|[\u4E00-\u9FFF]{0,3}主持者[:：]|起草人[:：]|参加指南讨论的专家[：:]).*)',r'\1删除12：<u>\2</u>'],
    ...
    #个例
    [r'(\n|^)(希望广大读者继续关注本刊，提出宝贵的意见和建议，积极提供文稿，本刊将择优刊登于相应栏目。|BA Bahia, CE MG Minas Gerais, PR Paraná, RJ Rio de Janeiro, SC Santa Catarina, SP São Paulo|Authors would to thank.*|The patient consented.*|The use of animals in this study.*|接受刊登的稿件.*|.*编写委员会成员[(（]按姓氏汉语拼音排序[）)][:：].*|.*执笔者[:：])',r'\1删除个例：<u>\2</u>']
]
...
fw = open(r"C:\Users\Administrator\PycharmProjects\untitled\paper-medjournals_zh\reclean3_paper-medjournals_zh.jsonl", "w", encoding="utf-8")
