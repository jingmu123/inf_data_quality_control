...
pattern_en_list=[
    [r'(\n|^)(Answered by *\n+\*+Dr\..*\*+ *\n+and.*)',r'删除1:<u>\1\2</u>'],
    [r'(\n|^)( *_This is a.*\d{4}_)', r'删除2:<u>\1\2</u>'],
    [r'(\n|^)(Answered by Dr\..*|#|(?:[Hh]i[,，] *|[Hh]ello[,，] *)?Wel+c?ome(?: back)? (?:to ?|in |of )*(?:the )?[Ii][cl][cl]in?i?q\.com *\.? *|You can always come back and reach me at icliniq\.com\. *|(?:Best |Kind )?[Rr]egards\.|Patient\'s Query|[Hh]i[,，] *|[Hh]ello[,，] *)',r'删除3:<u>\1\2</u>'],
    # [r'(\n|^)( *For (?:more|further) (?:information|queries|doubts) consult a.*)',r'删除4:<u>\1\2</u>'],
    # [r'(\n|^)( *Revert back with (?:the|an).*-->.*)',r'删除5:<u>\1\2</u>'],
    [r'(\n|^)(.*--> *https:.*)',r'删除4:<u>\1\2</u>'],
    [r'(\n|^)( *(?:https?[：:][/／]{2}(?:www[.．])?|www[.．])[a-zA-Z0-9@／/-]+(?:[.．／/][a-zA-Z0-9@／/_?=\-]{2,})+)',r'删除5:<u>\1\2</u>'],
    [R'(\n|^)( *(?:[welcome]|[WELCOME]){5,8} +(?:[back]|[BACK]){0,4} *(?:[tofin]|[TOFin]){2,4} *(?:[the]|[THE]){0,3} *(?:[icnql]|[ICNQL]){6,8}\.(?:[com]|[COM]){3} *\.? *)',r'删除3:<u>\1\2</u>']#删除3的补充
