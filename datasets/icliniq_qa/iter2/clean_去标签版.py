...
pattern_en_list=[
    [r'(\n|^)(Answered by *\n+\*+Dr\..*\*+ *\n+and.*)',r''],
    [r'(\n|^)( *_This is a.*\d{4}_)',r''],
    [r'(\n|^)(Answered by Dr\..*|#|(?:[Hh]i[,，] *|[Hh]ello[,，] *)?Wel+c?ome(?: back)? (?:to ?|in |of )*(?:the )?[Ii][cl][cl]in?i?q\.com *\.? *|You can always come back and reach me at icliniq\.com\. *|(?:Best |Kind )?[Rr]egards\.|Patient\'s Query|[Hh]i[,，] *|[Hh]ello[,，] *)',r''],
    # [r'(\n|^)( *For (?:more|further) (?:information|queries|doubts) consult a.*)',r''],
    # [r'(\n|^)( *Revert back with (?:the|an).*-->.*)',r''],
    [r'(\n|^)(.*--> *https:.*)',r''],
    [r'(\n|^)( *(?:https?[：:][/／]{2}(?:www[.．])?|www[.．])[a-zA-Z0-9@／/-]+(?:[.．／/][a-zA-Z0-9@／/_?=\-]{2,})+)', r''],
    [R'(\n|^)( *(?:[welcome]|[WELCOME]){5,8} +(?:[back]|[BACK]){0,4} *(?:[tofin]|[TOFin]){2,4} *(?:[the]|[THE]){0,3} *(?:[icnql]|[ICNQL]){6,8}\.(?:[com]|[COM]){3} *\.? *)',R'']#删除3的补充
]
