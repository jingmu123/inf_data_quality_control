...
pattern_en = [
    ...
    [
        ...
        r'通用删除1(英):<u>\1</u>'],  # 最广泛的形式从左括号匹配到右括号
    ...
    [r'(?<![\dm\s])(\s{0,}<sup>(<a>)?\s{0,}\d+[\d\s\–—,\(\)\[\]]{1,20}(</a>)?</sup>)', r'通用删除4(英):<u>\1</u>'],
    # 特殊数字  排除可能出现的次幂情况
    ...
    [
        ...
        r'通用删除8(英):<u>\1</u>'],  # 固定格式  带有[]的图片表格描述 附录描述 协议描述 无关网址描述
    ...
    [r'(^\s+Citations?:.*)',r'删除1:<u>\1</u>'],     # 引用...  由空格和单词Citation开头
    [r'(^Key Words:\s?\$.*\[Keywords.*)',r'删除2:<u>\1</u>'],   # Key Words: $\\[Keywords\\]固定格式
]
pattern_zh = [
...
ending_starts = [
    ...
    r'^(CONFLICTS? OF INTEREST STATEMENT|CONFLICTS?(ING)? OF INTEREST|Confict of Interest statement)',
    # 单行利益冲突CONFLICT OF INTEREST STATEMENT 全为大写
...
fw = open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\reclean1_wjcc_case.jsonl", "w", encoding="utf-8")
with open(r"C:\pycharm\orc识别pdf清洗数据\pdf\clean_json\original_data\wjcc_case_preformat.jsonl", "r", encoding="utf-8") as fs:
    lines = fs.readlines()
    for items in tqdm(lines):
        item = json.loads(items.strip())
        # if item["seq_id"] == "1c520bdf-0ac3-4407-a078-62e2d61f3167":
        context = item["text"]
        lang = item["lang"]
        title = item["title"]
